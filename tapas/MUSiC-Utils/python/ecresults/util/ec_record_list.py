...
from .class_count import ClassCountMixin

class ECRecordList(list, ClassCountMixin):
    ...
    def filter_not_in(self, key, value_list):
        return ECRecordList([r for r in self if r[key] not in value_list])
    ...
    def filter_skipped(self, invert=False):
        return ECRecordList([r for r in self if r["scan_skipped"] != (not invert) ])

    def filter_gt(self, key, val, allow_equal=False):
        if allow_equal:
            return ECRecordList([r for r in self if r[key] >= val])
        return ECRecordList([r for r in self if r[key] > val])

    def filter_lt(self, key, val, allow_equal=False):
        if allow_equal:
            return ECRecordList([r for r in self if r[key] <= val])
        return ECRecordList([r for r in self if r[key] < val])

    def filter_lt_by_keys(self, key1, key2, allow_equal=False):
        if allow_equal:
            return ECRecordList([r for r in self if r[key1] <= r[key2]])
        return ECRecordList([r for r in self if r[key1] < r[key2]])

    def filter_gt_by_keys(self, key1, key2, allow_equal=False):
        if allow_equal:
            return ECRecordList([r for r in self if r[key1] >= r[key2]])
        return ECRecordList([r for r in self if r[key1] > r[key2]])
    ...
    def get_class_type_counts(self, conf):
        counts = {}
        stats = self.get_class_type_dominant_process_stats(conf, 0)
    ...
    def get_class_type_dominant_process_stats(self, conf, quantile, **kwargs):
        return self.record_class_type_loop(self.get_dominant_process_stats,
                                           conf,
                                           quantile=quantile,
                                           distribution=self.distribution,
                                           **kwargs)

    def sort_records(self, sort_option):
        if sort_option == "objects":
            return ECRecordList(sorted( self, key=lambda k: ecroot.rank_by_objects(k["name"]), reverse=True))
        elif "integral" in sort_option:
             return ECRecordList(reversed(sorted( self, key=lambda k: k[ sort_option ] )))
        elif "data_integral" in sort_option:
             return ECRecordList(reversed(sorted( self, key=lambda k: k[ sort_option ] )))
        else:
            return ECRecordList(sorted( self, key=lambda k: k[ sort_option ] ))
