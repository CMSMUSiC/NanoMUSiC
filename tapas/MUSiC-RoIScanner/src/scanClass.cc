...
int readCommandLineOptions( int argc, char* argv[],
    ...
    ( "json,j", po::value<std::string>(&jsonFilePath)->required() ,
        "The input json file containing a vector of bin information" )
    ( "output,o", po::value<std::string>(&outputDirectory)->default_value( "." ) ,
        "Directory for output files (json, csv, root, ...). Files will still be prefixed with EC name and distribution." )
    ( "rounds,n", po::value<int>(&rounds)->default_value( 1 ) ,
        "Number of rounds to dice" )
    ( "start,l", po::value<int>(&startRound)->default_value( 0 ) ,
        "Number of round to skip in shifts file" )
...
int main(int argc, char* argv[]) {
    ...
    int success = readCommandLineOptions( argc,
                                          argv,
                                          jsonFilePath,
                                          outputDirectory,
                                          rounds,
