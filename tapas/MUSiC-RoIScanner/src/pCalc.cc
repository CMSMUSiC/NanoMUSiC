...
int main(int argc, char* argv[]) {
    ...
    po::options_description allOptions("Generic options");
        ...
        ( "batch", "only print p-value and return" )
        ( "normal", "use normal prior instead of log-normal" )
        ( "no-lookup", "skip the lookup table and perform full calculation" )
        ( "obs", po::value<double>(&N_obs)->required(),
            "observed event yield in this region" )
        ( "exp", po::value<double>(&N_exp)->required(),
            "expected event yield in this region" )
        ( "sigma-exp", po::value<double>(&sigma_exp)->required(),
            "total uncertainty on expected event yield" )
        ( "debug", po::value<int>(&debugLevel),
            "debuglevel: 0 = errors only, 1 = warnings, 2 = info, 3 = debug" );

    // parse command line options
    po::variables_map vm;
    try {
        po::store(po::command_line_parser(argc, argv).
             options(allOptions).run(), vm);
        if( vm.count("help") ){
            std::cout << "A simple tool to calculate p-values with a hybrid bayesian / frequentist approach" << std::endl;
