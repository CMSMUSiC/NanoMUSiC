...
public:
	...
	/// relations with the passed name \p name into the passed vector of Serializable pointers. 
	/// Returns the number of filled Serializables.
	int getSoftRelatives(std::vector<Serializable*>& vec, const BasicContainer& container, const std::string& name) const;
	...
	/// Returns the number of filled objects.
	template <class objecttype>
	int getSoftRelativesOfType(std::vector<objecttype*>& vec, const BasicContainer& owner, const std::string& name) const;
...
int SoftRelations::getSoftRelativesOfType(std::vector<objecttype*>& vec,
		const ObjectOwner& owner, const std::string& name) const
{
	int size = vec.size();
	std::pair<const_iterator, const_iterator> iterators =
			_relationsMap.equal_range(name);
...
template<class objecttype>
int SoftRelations::getSoftRelativesOfType(std::vector<objecttype*>& vec,
		const BasicContainer& owner, const std::string& name) const
{
	int size = vec.size();
	std::pair<const_iterator, const_iterator> iterators =
			_relationsMap.equal_range(name);
