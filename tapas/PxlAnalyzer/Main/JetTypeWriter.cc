...
// Constructor
JetTypeWriter::JetTypeWriter(const Tools::MConfig &cfg) : m_data(cfg.GetItem<bool>("General.RunOnData")),
...
double JetTypeWriter::getBTagScaleFactor(const pxl::Particle *object) const
{
   ...
   double bTagScaleFactor = m_corrector.at(hadronFlavour)->evaluate({
       "central",              // systematics
...
std::pair<double, double> JetTypeWriter::getBTagScaleFactorError(const pxl::Particle *object) const
{
   ...
   bTagScaleFactor_up = m_corrector.at(hadronFlavour)->evaluate({
       "up",                   // systematics
       m_bJet_discriminatorWP, // working point
       hadronFlavour,          // hadron flavor (5 (btag), 4 (ctag) or 0 (light))
       std::abs(eta),          // |eta|at(
   ...
   bTagScaleFactor_do = m_corrector.at(hadronFlavour)->evaluate({
       "down",                 // systematics
       m_bJet_discriminatorWP, // working point
       hadronFlavour,          // hadron flavor (5 (btag), 4 (ctag) or 0 (light))
       std::abs(eta),          // |eta|
