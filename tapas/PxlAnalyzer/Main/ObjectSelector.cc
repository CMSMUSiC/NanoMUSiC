...
bool ObjectSelector::passKinematics( pxl::Particle *object, const bool isSyst ) const{
    ...
    bool isBarrel = false;
    bool isEndcap = false;
    if( abseta <  eta_barrel_max ) isBarrel = true;
    if( m_object_endcap_eta_cuts &&
        !isBarrel &&
        abseta > eta_endcap_min &&
        abseta <  eta_endcap_max ) isEndcap = true;

    object->setUserRecord( "isBarrel", isBarrel );
    object->setUserRecord( "isEndcap", isEndcap );

    if( isBarrel and m_object_endcap_only ) return false;
    if( isEndcap and m_object_barrel_only ) return false;

    if( m_object_endcap_eta_cuts && !isBarrel && !isEndcap ) return false;
    if( !m_object_endcap_eta_cuts && !isBarrel ) return false;

    if( object->getPt()  <  pt_min ) return false;
...
void ObjectSelector::setScaleFactors( pxl::Particle *object ){
    ...
    for( auto& sf : m_scale_factors ){
        ...
        if ( ( sf.getName().find( "ID." ) != std::string::npos ) && // check if this is an combined ID related SF
             ...
             ( sf.getName() != ( "ID." + used_id ) ) ){ // check if this SF does not match the ID for the current object
            continue;
        }
        if ( sf.getName().find( "Iso." ) != std::string::npos && // check if this is an combined ID related SF
             used_id.find( "None" ) == std::string::npos &&// check if this object matches no ID in combined ID
             sf.getName() != ( "Iso." + used_id ) ){ // check if this SF does not match the ID for the current object
            continue;
        }
