...
bool EventSelector::passFilterSelection( pxl::EventView *EvtView ) {
   ...
   for (auto& element : m_all_filters_map) {
      ...
      if( !EvtView->hasUserRecord( filter ) ) continue;
      // check filter result
...
void EventSelector::applyKinematicCuts( std::vector< pxl::Particle* > &objects,
   ...
   for(vector< Particle* >::const_iterator object = objects.begin(); object != objects.end(); ++object) {
      ...
      if( selector_map[ partName ]->passKinematics( *object, isSyst) ){
         ...
         if( ( !isSyst and (*object)->hasUserRecord( "systIDpassed" ) and
               (*object)->getUserRecord( "systIDpassed" ).toBool() )
              or
             isSyst ){
   ...
   if( !particleTagMap[ partName ] ) objects = objectsAfterCut;
}
...
std::map< std::string, int > EventSelector::getParticleCountMap( const std::map< std::string, std::vector< pxl::Particle* > >& particleLists ) const {
...
//std::map< std::string, std::vector< pxl::Particle* > > EventSelector::getGenParticleLists ( const pxl::EventView* EvtView ) const {
...
std::map< std::string, std::vector< pxl::Particle* > > EventSelector::getParticleLists ( const pxl::EventView* EvtView,
                                                                                         bool selectedOnly,
...
void EventSelector::performKinematicsSelection( pxl::EventView* EvtView, const bool useSystCuts ){
   ...
   for( auto& part_name_use : particleUseMap ){
      if( part_name_use.second ) applyKinematicCuts( particleMap[ part_name_use.first ],
                                                     part_name_use.first,
                                                     useSystCuts
                                                     );
   }
...
void EventSelector::setScaleFactors( std::vector< pxl::Particle* > &objects, const std::string& partName ){
   for( auto& obj : objects ) selector_map[ partName ]->setScaleFactors( obj );
}
...
void EventSelector::performOfflineTriggerSelection( pxl::EventView* RecEvtView, pxl::EventView *TrigEvtView ) const
{
   ...
   {
      std::string err = "EventSelector::performOfflineTriggerSelection: No userRecord 'HLT_accept_no_offline_cuts' found!\n";
...
void EventSelector::performSelection(pxl::EventView* RecEvtView,
                                     ...
                                     const bool useSystCuts ) {   //used with either GenEvtView or RecEvtView
   ...
   for( auto& part_name_use : particleUseMap ){
      if( part_name_use.second ) applyCutsOnObject( particleMap[ part_name_use.first ],
                                                    part_name_use.first,
   ...
   if( not m_data ){
      for( auto&  part_name_use : particleUseMap ){
         if( particleUseMap[ part_name_use.first ] ) setScaleFactors( particleMap[ part_name_use.first ], part_name_use.first );
         if ( part_name_use.first .find( "Jet" ) != std::string::npos )//sarFIXME check //separate method for BTag scale factor
            m_typeWriter.setBTagScaleFactor( particleMap[ part_name_use.first ], RecEvtView->getUserRecord( "EventSeed" ).toInt32());
      }
   }
...
void EventSelector::removeOverlaps( pxl::EventView* RecEvtView ) const
{
   ...
   if( not m_ignoreOverlaps ) m_eventCleaning.cleanEvent( particleMapSelectedOnly,
