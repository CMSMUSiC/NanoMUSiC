...
GenSelector::GenSelector( Tools::MConfig const &cfg ) :
   ...
   m_mass_double_types_first( Tools::splitString< std::string >( cfg.GetItem< std::string >( "Generator.Mass.Types.double.first" ), true  ) ),
   m_mass_double_types_second( Tools::splitString< std::string >( cfg.GetItem< std::string>( "Generator.Mass.Types.double.second" ), true  ) ),
   ...
   m_pt_double_types_first( Tools::splitString< std::string >( cfg.GetItem< std::string >( "Generator.pt.Types.double.first" ), true  ) ),
   m_pt_double_types_second( Tools::splitString< std::string >( cfg.GetItem< std::string>( "Generator.pt.Types.double.second" ), true  ) ),
...
bool GenSelector::passHtCuts( pxl::EventView const *EvtView ) const {
   ...
   if( m_ht_min <= 0 and m_ht_max <= 0 ) return true;
...
bool GenSelector::passTransverseMomentumCuts( std::vector< pxl::Particle* > genParts ) const {
   ...
   try{
   	   ...
   	   if(m_ignore_missing_particle){
		   std::cout<<" Caught exception : "<<exception.what()<<" : Continuing anyway due to flag m_ignore_missing_particle"<<std::endl;
   		   return true;
	   }
   	   throw exception;
   }
...
std::vector< pxl::Particle* > GenSelector::getGenParticles( pxl::EventView* GenEvtView, std::string gen_label )const{
    ...
    for ( std::vector<pxl::Particle*>::const_iterator part = allparticles.begin(); part != allparticles.end(); ++part ) {
		...
		if( name == gen_label ) genParticles.push_back( *part );
    }
    return genParticles;
}
...
pxl::LorentzVector GenSelector::getGenCutObject( std::vector< pxl::Particle* > genParts, const std::string& type)const{
   ...
   for (std::vector<pxl::Particle*>::const_iterator part = genParts.begin(); part != genParts.end(); ++part) {
      ...
      if( use_single and
         std::find( single_types.begin(), single_types.end(), pdg_id_map[ abs( pdgId ) ] ) != single_types.end() ) return (*part)->getVector();
      if( use_double and
         firstPart != 0 and
         secondPart == 0 and
         std::find( double_types_second.begin(), double_types_second.end(), pdg_id_map[ abs( pdgId)  ] ) != double_types_second.end() ){
         secondPart = *part;
      }
      if( use_double and
         firstPart == 0 and
         std::find( double_types_first.begin(), double_types_first.end(), pdg_id_map[ abs( pdgId ) ] ) != double_types_first.end() ){
         firstPart = *part;
     }
      // gen particles are sorted from left to right in Feynman graph. Stop if we found to particles
      if( firstPart != 0 and secondPart != 0 ) break;
   }
...
bool GenSelector::passGammaCuts( pxl::EventView const *EvtView,
   ...
   for( auto &gam  : particleLists[ "Gamma" ] ){
      ...
      for(auto mother_relative : gam->getMothers()){
		 ...
		 //sar change
         if( mother->getPdgNumber() <= 9 ||  mother->getPdgNumber() == 21 || mother->getPdgNumber() == 22 || mother->getPdgNumber() == 23 || mother->getPdgNumber() == 2212 ){
		 //if( mother->getPdgNumber() <= 9 ||  mother->getPdgNumber() == 21 || mother->getPdgNumber() == 23 || mother->getPdgNumber() == 2212 ){
			 //if( mother->getPdgNumber() == 23 ){
         //if( mother->getPdgNumber() <= 9 ||  mother->getPdgNumber() == 21 || mother->getPdgNumber() == 22 || mother->getPdgNumber() == 2212 ){
            valid_mother = true;
            break;
         }
      }
      if(! valid_mother) continue;
...
bool GenSelector::passDYGammaCuts( pxl::EventView const *EvtView,
   ...
   for (std::vector<pxl::Particle*>::const_iterator part = genParts.begin(); part != genParts.end(); ++part) {
	  ...
	  if(photon_likeZGamma) break;
	  if( (*part)->getUserRecord( "Status" ).toInt32() == 1 ){
		  pdgId = (*part)->getPdgNumber();
		  if(pdgId != 22) continue;
		  if( (*part)->getPt() < m_gamma_genPtCut) continue; 
                  if( !((*part)->getUserRecord( "isPromptFinalState" )) ) {
                          continue; //photon_likeZGamma = true;
                  }		  
		  for(auto mother_relative : (*part)->getMothers()){
			  auto mother = dynamic_cast<pxl::Particle *>( mother_relative );
			  if( std::abs (mother->getPdgNumber()) <= 21 ||  mother->getPdgNumber() == 22 || mother->getPdgNumber() == 23 || mother->getPdgNumber() == 2212 ){
