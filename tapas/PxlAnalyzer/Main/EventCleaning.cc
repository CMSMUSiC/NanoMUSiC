...
EventCleaning::EventCleaning( Tools::MConfig const &cfg ) :
   ...
   {{ "Muon", &EventCleaning::cleanMuos },
   { "Ele", &EventCleaning::cleanEles },
   { "Gamma", &EventCleaning::cleanGams }} ),
   m_deltaR_map(
   {{ "Muon", cfg.GetItem< double >( "Muon.DeltaR.max" ) },
...
void EventCleaning::cleanEvent( std::map< std::string, std::vector< pxl::Particle* > >& particleMap,
                                bool const isRec
                                ) const {
   ...
   for ( auto& name : m_self_cleaning )
   {
      if ( m_particleUseMap.find( name ) == m_particleUseMap.end() )
      {
         ...
         error << "[ERROR](EventCleaning::cleanEvent " << __LINE__ << ") Did not find particle '" << name << "' in particle use map." << std::endl;
      ...
      if ( m_particleUseMap.at( name ) )
      {
         ...
         {
            std::stringstream error;
            error << "[ERROR](EventCleaning::cleanEvent " << __LINE__ << ") Did not find particle '" << name << "' in particle map." << std::endl;
            throw( std::runtime_error( error.str() ) );
         }
         ...
         {
            std::stringstream error;
            error << "[ERROR](EventCleaning::cleanEvent " << __LINE__ << ") Did not find self cleaning function for '" << name << "'." << std::endl;
            throw( std::runtime_error( error.str() ) );
         }
      }
   }
...
std::vector< pxl::Particle* >::iterator EventCleaning::removeParticle( std::vector< pxl::Particle* > &particles,
...
int EventCleaning::checkGammaOverlap( pxl::Particle const *paI,
   ...
   // Check DeltaR.
   if( checkParticleOverlap( paI, paJ, m_deltaR_map.at( "Gamma" ) ) and
