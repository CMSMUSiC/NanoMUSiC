...
void Splitting::split( std::map< std::string, std::vector< pxl::Particle* > >& map, const std::function< bool( pxl::Particle* ) >& splittingFunc, const std::string& origName, const std::string& name1, const std::string& name2 )
{
...
void Splitting::merge( std::map< std::string, std::vector< pxl::Particle* > >& map, const std::vector< std::string >& names, const std::string& newName )
{
   for ( auto& name : names )
   {
...
void Splitting::merge( std::map< std::string, std::vector< pxl::Particle* > >& map, const std::string& name1, const std::string& name2, const std::string& newName )
{
...
{
   split( map, [] ( pxl::Particle* particle ) {
      return particle->hasUserRecord( "isBjet" ) && particle->getUserRecord( "isBjet" ); },
      "Jet", "bJet", "Jet" );
}
...
{
   split( map, [] ( pxl::Particle* particle ) {
      return particle->hasUserRecord( "isWjet" ) && particle->getUserRecord( "isWjet" ); },
      "FatJet", "wJet", "FatJet" );
}
...
void Splitting::mergeZ( std::map< std::string, std::vector< pxl::Particle* > >& map, double deltaM, std::list< std::string > particles, std::string newName )
{
   ...
   for ( auto& particle : particles )
   {
      ...
      {
         std::stringstream error;
         error << "[ERROR](Splitting::mergeZ) Did not find particle '" << particle << "' in particle map." << std::endl;
         throw( std::runtime_error( error.str() ) );
      }
