...
Systematics::Systematics(const Tools::MConfig &cfg, unsigned int const debug):
   ...
   m_fatjecPara( Tools::ExpandPath( cfg.GetItem< std::string >( "FatJet.Error.JESFile" ) ), m_jecType ),
   m_jecUnc( m_jecPara ),
   m_fatjecUnc( m_fatjecPara ),
   m_jetRes( cfg, "Jet" ),
...
void Systematics::init(pxl::Event* event){
   ...
   for( auto& part : AllParticles ){
      ...
      if( m_particleUseMap[ m_particleRecoNameMap[ partName ] ]){
         if( !( part->getUserRecord( "systIDpassed" ).toBool() ) ) continue;
   ...
   for( auto& part: GenParticles ) {
      ...
      if( Name == m_METType+"uncert_10") UnclusteredEnUp = part;
      else if( Name == m_METType+"uncert_11") UnclusteredEnDown = part;
      persistent_id = Tools::random_string( 16 );
      part->setUserRecord("persistent_id", persistent_id );
  }
...
void Systematics::shiftBJetsSF2A( std::string const& partName,
   ...
   if ( not m_emptyShift and m_particleMap[ partName ].size() == 0 ) return;
...
std::pair<double,double> Systematics::getObjScaleShift(pxl::Particle* obj,
   ...
   if( partName == "Muon"){
      ...
      double uncert = rand->Gaus(shift, uncertainty)
                     * obj->getPt() / 1000.
                     / obj->getCharge();
   ...
   /////////
   if( partName == "Jet" ){
      m_jecUnc.setJetEta( obj->getEta() );
      m_jecUnc.setJetPt( obj->getPt() );
      double ratio = m_jecUnc.getUncertainty( true ) ;
      return std::make_pair( 1. + ratio, 1. - ratio);
   }

   if( partName == "FatJet" ){
      m_fatjecUnc.setJetEta( obj->getEta() );
      m_fatjecUnc.setJetPt( obj->getPt() );
      double ratio = m_fatjecUnc.getUncertainty( true ) ;
      return std::make_pair( 1. + ratio, 1. - ratio);
   }

   // Final check for Gammas and eles
   if( no_region ){
      throw std::runtime_error( std::string("Systematics.cc:") + partName
...
std::pair<double,double> Systematics::getObjResolutionShift(pxl::Particle* obj, std::string const& partName){
   if( partName == "Muon"){
     ...
     if (std::abs(obj->getEta()) > 1.2) smearing *= 2;
   ...
   if( partName == "Ele"){
     ...
     if (std::abs(obj->getEta()) > 1.5) smearing *= 2;
     // Return Gaussian smearing
     double ratio = rand->Gaus(0, smearing);
     return std::make_pair(1 + ratio, 1.);
   }
