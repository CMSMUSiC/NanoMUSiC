...
ParticleMap::ParticleMap( const std::map< std::string, std::vector< pxl::Particle* > >& particleMap ) :
   ...
   { "MET", Resolutions::met }} ),
   m_funcMap (
   {{ "pt", std::bind( &pxl::Particle::getPt, std::placeholders::_1 ) },
...
void ParticleMap::split( std::vector< pxl::Particle* > particles, std::function< bool( pxl::Particle* ) > splittingFunc, const std::string name1, const std::string name2 )
{
   std::vector< pxl::Particle* > first = std::vector< pxl::Particle* >();
   std::vector< pxl::Particle* > second = std::vector< pxl::Particle* >();
   for ( auto particle : particles )
   {
      if ( splittingFunc( particle ) ) first.push_back( particle );
      else second.push_back( particle );
   }
...
double ParticleMap::getScaleFactorError( const std::string error_type,
                                         const std::map< std::string, int > &countMap ) const
{
...
std::map< std::string, int > ParticleMap::getFakeMap( const std::map< std::string, int > &countMap ) const
{
   ...
   for ( auto& partVec : m_map )
   {
      ...
      for ( auto& part : partVec.second.getParticles() )
      {
         ...
         if ( counter > countMap.at( partVec.first ) )
            break;
         if ( part->hasUserRecord( "Match" ) and part->getUserRecord( "Match" ).toInt32() == -1 ) fakeMap[ partVec.first ]++;
      }
   }
   return fakeMap;
}
...
std::map< std::string, int > ParticleMap::getChargeFakeMap( const std::map< std::string, int > &countMap ) const
{
   ...
   for ( auto& partVec : m_map )
   {
      ...
      for ( auto& part : partVec.second.getParticles() )
      {
         ...
         if ( counter > countMap.at( partVec.first ) )
            break;
         if ( part->hasUserRecord( "ChargeMatch" ) and part->getUserRecord( "ChargeMatch" ).toInt32() == -1 ) fakeMap[ partVec.first ]++;
      }
   }
   return fakeMap;
}
                                                  ...
                                                  double const fudge ) const
{
   ...
   if( min < 0 ) throw std::range_error( "min must be >= 0!" );
   if( max <= 0 ) throw std::range_error( "max must be > 0!" );
   if( max <= min ) throw std::range_error( "max must be > min!" );
   if( step_size <= 0 ) throw std::range_error( "step_size must be > 0!" );
   ...
   if ( distribution == "MET" )
   {
      resfunc = [this] (std::map< std::string, int > countMap, double sumpt, double const fudge ) { return getApproximateResolutionMET( countMap, sumpt, fudge ); };
   }
   else
   {
      resfunc = [this] (std::map< std::string, int > countMap, double sumpt, double const fudge ) { return getApproximateResolution( countMap, sumpt, fudge ); };
   }
   ...
   while ( next_value < max ) {
      ...
      while ( ( next_value - last_value ) < resfunc( countMap,
                                                    ( next_value + last_value ) / 2,
                                                    fudge
                                                    )
            )
      {
         if ( next_value >= max ) break; //stop in case we ran over the end
         else next_value += step_size;
      }
      if ( next_value >= max ) break; //we are done once we are at or beyond the end
      else
      {
...
double ParticleMap::getApproximateResolution( const std::map< std::string, int > &countMap, double sumpt, double const fudge ) const
{
   double res = 0;
   int numObjects = 0;
...
double ParticleMap::getApproximateResolutionMET( const std::map< std::string, int > &countMap, double sumpt, double const fudge ) const
{
