...
AnalysisComposer::AnalysisComposer( ) :
   m_analysisName("MUSiC Classification"),
...
po::options_description AnalysisComposer::getCmdArguments( ){
   ...
   myoptions.add_options()
      ( "XSections,x", po::value<string>(&m_XSectionsFile),
                      "Path to cross-sections file.")
      ("outfilename,e",po::value<string>(&m_outfilename), "Name of the outpufile.\
                         Constructed from process if omitted")
      ( "hash", po::value<string>(&m_RunHash),
                      "Path to cross-sections file.");
      return myoptions;
}
