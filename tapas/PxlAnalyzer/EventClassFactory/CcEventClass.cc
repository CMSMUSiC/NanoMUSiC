...
void splitGammaRegions(std::map< std::string, std::vector< pxl::Particle* > >& map){
   return Splitting::split( map, [] ( pxl::Particle* particle ) {
...
CcEventClass::CcEventClass( const Tools::MConfig &cutconfig,
                            const Tools::MConfig &xsecs,
   ...
   m_jet_bJets_algo(      cutconfig.GetItem< string >( "Jet.BJets.Algo" ) ),
...
std::map< std::string, double > CcEventClass::readFakeErrorMap( const Tools::MConfig &cutconfig ){
...
std::map< std::string, double > CcEventClass::readChargeErrorMap( const Tools::MConfig &cutconfig ){
...
void CcEventClass::addConstantSystematics(){
   ...
   systematics->m_activeSystematics.push_back( new SystematicsInfo( "General", getQCDSystWeightName("Up"), getQCDSystWeightName("Up") , false ) );
   systematics->m_activeSystematics.push_back( new SystematicsInfo( "General", getQCDSystWeightName("Down"), getQCDSystWeightName("Down"), false ) );
   systematics->m_activeSystematics.push_back( new SystematicsInfo( "General", "scale_factor_syst", "scale_factor_syst", false ) );
...
std::string CcEventClass::getQCDSystWeightName(std::string shift_type) const {
   return "qcdWeight" + m_lastprocessGroup + "_" + shift_type;
}

void CcEventClass::FinishEventClasses( map< std::string, TEventClass* > &EvtClasses,
   ...
   for (map<std::string, TEventClass*>::iterator Iter = EvtClasses.begin(); Iter != EvtClasses.end(); ++Iter) {
      ...
      if( !m_data ){
         if( m_usePDF ) Iter->second->calculatePDFUncertainty(); // Perform PDF Reweighting
...
void CcEventClass::endJob( const Serializable* ) {

   #ifdef TRIGGERSTREAM
   trig_stream.writeUniqueTriggers();
   #endif

   // cutflows in seperate root file
   TFile* fout_cutflow =  new TFile( "cutflow.root", "RECREATE" );
   if ( !m_data && !eventCounts.empty() )
   {
      std::string proc = eventCounts.begin()->first;
      double const xsec = m_XSections.GetItem< double >( proc + ".XSec" );
      double const fEff = m_XSections.GetItem< double >( proc + ".FilterEff" );
      double const kFac = m_XSections.GetItem< double >( proc + ".kFactor" );
      double const totalXsec = xsec * fEff * kFac;
   ...
   std::set< std::string > ProcessList;
   for( map< std::string, unsigned int >::const_iterator proc_num = eventCounts.begin(); proc_num != eventCounts.end(); ++proc_num ) {
   ...
   for (map<std::string, TEventClass*>::iterator Iter = _recEventClasses.begin(); Iter != _recEventClasses.end(); Iter++) {
      if (Iter->second != 0) {
         delete Iter->second;
      }
   }
   for (map<std::string, TEventClass*>::iterator Iter = _recInclEventClasses.begin(); Iter != _recInclEventClasses.end(); Iter++) {
      if (Iter->second != 0) {
         delete Iter->second;
      }
   }
   for (map<std::string, TEventClass*>::iterator Iter = _recJetInclEventClasses.begin(); Iter != _recJetInclEventClasses.end(); Iter++) {
      if (Iter->second != 0) {
         delete Iter->second;
      }
   }
   if( ! m_data ){
      for (map<std::string, TEventClass*>::iterator Iter = _genEventClasses.begin(); Iter != _genEventClasses.end(); Iter++) {
         if (Iter->second != 0) {
            delete Iter->second;
         }
      }
      for (map<std::string, TEventClass*>::iterator Iter = _genInclEventClasses.begin(); Iter != _genInclEventClasses.end(); Iter++) {
         if (Iter->second != 0) {
            delete Iter->second;
         }
      }
      for (map<std::string, TEventClass*>::iterator Iter = _genJetInclEventClasses.begin(); Iter != _genJetInclEventClasses.end(); Iter++) {
         if (Iter->second != 0) {
            delete Iter->second;
         }
      }
...
void CcEventClass::prepareProcessName( std::string& proc ){
...
void CcEventClass::CheckForCrossSection( string& process ){
   ...
   if( m_lastprocess.compare( processKey ) != 0 ) {
      if( !m_XSections.CheckItem( processKey ) ) {
         throw std::runtime_error("CcEventClass::CheckForCrossSection( string process ): No value found for item: '" +
               processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      if( !m_XSections.CheckItem( processKey ) ) {
         throw std::runtime_error("CcEventClass::CheckForCrossSection( string process ): No XSecOrder found for item: '" +
               processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      if( !m_XSections.CheckItem( processGroupKey ) ) {
         throw std::runtime_error("CcEventClass::CheckForCrossSection( string process ): No processGroup found for item: '" +
               processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      m_lastprocess = process;
      m_lastprocessOrder = m_XSections.GetItem< string >( processOrderKey ) ;
      if (m_lastprocessOrder == "")
      {
        cerr << "No process order set in scales.txt" << endl;
        exit(1);
      }
      std::string newprocessGroup = m_XSections.GetItem< string >( processGroupKey );
      if (newprocessGroup == ""){
         newprocessGroup = m_lastprocess;
      }
      m_xsec = m_XSections.GetItem< double >( process + ".XSec" );
      m_fEff = m_XSections.GetItem< double >( process + ".FilterEff" );
      m_kFac = m_XSections.GetItem< double >( process + ".kFactor" );
      if(newprocessGroup != m_lastprocessGroup and newprocessGroup.find("uninitialized") != string::npos){
         throw std::runtime_error("CcEventClass, trying to mix samples from different process groups in one run");
      }
      m_lastprocessGroup = newprocessGroup;
   }
}

void CcEventClass::fillFilterCutFlow(const double weight){
...
void CcEventClass::fillCutFlow( const pxl::Event* event, const double weight )
{
...
void CcEventClass::fillCutFlow( const double sumPt,
...
void CcEventClass::analyseEvent( const pxl::Event * event ) {
...
bool CcEventClass::hasTooManyJets(std::map< std::string, int > countMap){
   unsigned int njets = countMap["Jet"] + countMap["bJet"];
   return njets > m_max_jet_ec;
}

//----------------------------------------------------------------------
void CcEventClass::Fill( pxl::EventView *EvtView,
                         double const weight,
                         EventInfo &event_info,
                         std::string systName
                         ) {
   // MET filters
   if( !EvtView->getUserRecord( "filter_accept" ) ) return;
   // Cross cleaning of trigger streams
   if( EvtView->getUserRecord( "Veto" ) ) return;
   // Trigger string and offline cuts fullfiled for one trigger
   if( !EvtView->getUserRecord( "trigger_accept" ) ) return;
   // Generator cuts
   if( !EvtView->getUserRecord( "generator_accept" ) ) return;

   // simple prefiring check
   if(m_use_prefire_check && !EvtView->getUserRecord("pass_prefiring_check")) return;

   std::string type = EvtView->getUserRecord( "Type" );
   if( ( type == "Gen" and m_ec_recOnly ) or ( type == "Rec" and m_ec_genOnly ) ) return;
   ...
   if( incl_empty == 0 ) {
      ...
      incl_empty = InitTEventClass( type,
                                    incl_empty_name,
                                    particleMap,
   ...
   if(!hasTooManyJets(countMap)){
      ...
      if( EventClasstoFill == 0) {
         // EventClass not existing, create it
         EventClasstoFill = InitTEventClass( type,
                                             excl_EC_name,
                                             particleMap,
                                             countMap,
                                             absCharge,
      ...
      if( m_data ){
         ...
         FillEventClass( EventClasstoFill,
                         event_info,
                         processName,
                         particleMap,
                         countMap
...
void CcEventClass::FillInclusiveRecursive( std::map< std::string, TEventClass* > &inclEventClasses,
   ...
   for( int partCount = recCountMap[ partName ] ; partCount <= refCountMap[ partName ]; partCount++){
      ...
      if(printIt){
          ...
          for(auto countMap : refCountMap ) std::cout << countMap.second << countMap.first<<" ";
          std::cout << std::endl <<"recMap ";
          for(auto countMap : recCountMap ) std::cout << countMap.second << countMap.first<<" " ;
      }
      if( partName == partNameVector.back() ){
         bool allEmpty = true;
         //check if recCountMap is empty
         for( auto count : recCountMap ){
            if (count.second > 0 ) allEmpty = false;
         }
         ...
         EventClassName = TEventClass::calculateEventClass( recCountMap, std::map< std::string, int >(), orderParticleTypes ) + nameSuffix;
         ...
         if(!hasTooManyJets(recCountMap)){
            ...
            if( m_data ){
               //data mode, so there are no weights at all
               FillEventClass( InclEventClasstoFill,
                               event_info,
                               Process,
                               particleMap,
                               recCountMap
      ...
      }else{ // we are not at the deepest level yet
         FillInclusiveRecursive( inclEventClasses,
                                 partNameVector,
                                 Type,
                                 Process,
...
// the destructor of CcEventClass.
...
// empty).
TEventClass *CcEventClass::InitTEventClass( std::string const &Type,
                                            std::string const &ECName,
                                            ParticleMap &particleMap,
                                            std::map< std::string , int > countMap,
                                            int const absCharge,
                                            bool const inclusiveEC
                                            ) const {
   // Calculate bin edges, depending on resolution.
   std::vector< double > const sumptBins = particleMap.getBinLimits( "SumPt",
                                                                     countMap,
                                                                     0,
                                                                     m_cme,
                                                                     m_bin_size_min,
   ...
   std::vector< double > const minvBins = particleMap.getBinLimits( "InvMass",
                                                                    countMap,
                                                                    0,
                                                                    m_cme,
                                                                    m_bin_size_min,
   ...
   std::vector< double > const metBins = particleMap.getBinLimits( "MET",
                                                                   countMap,
                                                                   0,
                                                                   m_cme,
                                                                   m_bin_size_min,
   ...
   if ( countMap[ "MET" ] > 0 and ECName.find( "Empty" ) == std::string::npos ) distTypeBins.emplace( "MET" , metBins );
   ...
   // Tell CcEventClass what systematics should be expected
   ...
   for( auto& systInfo : systematics->m_activeSystematics ){
      ...
      if( !systInfo->m_isDifferential ) systNames.emplace( systInfo->m_funcKey );
      else{
         for( auto& evtView : systInfo->eventViewPointers ){
   ...
   return new TEventClass( Type,
                           ...
                           distTypeMinsRequire,
                           m_XSections.GetItem< double >( "Lumi" ),
                           systNames
                           );
}

void CcEventClass::FillEventClass( TEventClass *EventClassToFill,
                                   EventInfo &event_info,
   ...
   if( m_data or !systName.empty() ) minimalFill = true;
   ...
   if( sumPt < m_ec_sumpt_min ) return;
   if( numPart > 1 and Minv < m_ec_minv_min ) return;
   if( countMap[ "MET" ] > 0 and MET < m_ec_met_min ) return;
   ...
   values[ "SumPt" ] = sumPt;
   values[ "InvMass" ] = Minv;
   if( countMap[ "MET" ] > 0) values[ "MET" ] = MET;
   ...
   if( m_data ) {
      ...
      if( numPart > 0 and CheckEventToList( sumPt, Minv, MET ) ) {
   ...
   } else if( !systName.empty() ){ // Fill differential systematic
        ...
        EventClassToFill->FillDifferentialSystematic( process,
                                                      values,
                                                      weight,
                                                      m_pdfTool.getPDFWeights(),
                                                      systName );
    }
    else {
      ...
      systWeights[ "luminosity" ] = 1 + m_XSections.GetItem< double >( "Global.ScalefactorError" );
      ...
      if ( event_info.has_scale_variation ) {
         ...
         while ( pos2 != std::string::npos && pos1 != event_info.qcd_scale.length())
         {
            ...
            qcdScaleWeights.push_back( std::stof( event_info.qcd_scale.substr( pos1, pos2 - pos1 ) ) / event_info.central_weight );
...
bool CcEventClass::CheckEventToList( double const sumPt,
                                     double const Minv,
                                     double const MET
                                     ) const {
   // Simply accept all events with anything in them, atm.
   if( sumPt > 0.0 or Minv > 0.0 or MET > 0.0 ) return true;
   return false;
}


void CcEventClass::FillEventList( std::string const &ECname,
                                  EventInfo const &event_info
                                  ) {
...
std::vector< CcEventClass::EventInfo > CcEventClass::GetListedEvents( std::string const &ECname ) {
...
void CcEventClass::WriteEventListToFile( std::string const &ECname ) {
   ...
   outfile << "#"
           ...
           << std::setw( 11 ) << "met:"
           << std::setw( 11 ) <<  "run:"
           << std::setw( 15 ) << "lumi section:"
           << std::setw( 16 ) << "event number:"
           << std::setw( 72 ) << "pxlio file:"
           << std::setw( 9 ) << "num:"
           << std::endl;
   outfile << std::string( 160, '#') << endl;
   ...
   for( event_it = event_list.begin(); event_it != event_list.end(); ++event_it ) {
      ...
      outfile << std::setw( 13 ) << event_it->sumpt         << "  "
              << std::setw( 11 ) << event_it->minv          << "  "
              << std::setw( 9 )  << event_it->met           << "  "
              << std::setw( 9 )  << event_it->run           << "  "
              << std::setw( 13 ) << event_it->lumisection   << "  "
              << std::setw( 14 ) << event_it->eventnum      << "  "
              << std::setw( 70 ) << event_it->filename      << "  "
              << std::setw( 7 )  << event_it->eventnumpxlio << std::endl;
      counter++;;
   }
   outfile.close();
}
...
void CcEventClass::WriteAnalysisInfoToFile( std::set< std::string > const &processList ) {
   ...
   if( m_debug > 0 ) {
      std::cerr << std::endl;
      std::cerr << "[INFO] (CcEventClass):" << std::endl;
