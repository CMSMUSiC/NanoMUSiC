// from: https://github.com/ECOP/cmg-cmssw/blob/heppy_80X_M17/PhysicsTools/Heppy/src/IsolationComputer.cc
// https://github.com/ECOP/cmg-cmssw/commit/80e9b20a7ef64eae1ac25e0d281553c1f353df78
...
/// Isolation from charged from the PV
float heppy::IsolationComputer::getAbsMiniIso(const reco::Candidate &cand, const double rho, bool muon) {
    double miniIsoConeSize = getMiniIsoConeSize(cand.pt());
    double miniAbsIsoCharged = -999.;
    double miniAbsIsoNeutral = -999.;
    if (muon) {
      miniAbsIsoCharged = chargedAbsIso(cand, miniIsoConeSize, 0.0001, 0.0);
      miniAbsIsoNeutral = neutralAbsIsoRaw(cand, miniIsoConeSize, 0.01, 0.5);
    }
    double effArea = getEA(cand, muon);
    miniAbsIsoNeutral = std::max(0.0, miniAbsIsoNeutral - rho * effArea * std::pow(miniIsoConeSize/0.3, 2.));
    return miniAbsIsoCharged + miniAbsIsoNeutral;
}

float heppy::IsolationComputer::getMiniIsoConeSize(const double pt) {
  return 10.0/(std::min(std::max(pt, 50.),200.));
}

const double heppy::IsolationComputer::getEA(const reco::Candidate &cand, bool muon) {
  // helper function to get effective areas for muons
  double etaAbs = fabs(cand.eta());
