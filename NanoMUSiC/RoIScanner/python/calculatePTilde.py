...
def main():
    ...
    with dbtools.session_scope(engine) as session:
        ...
        if compare_scan.scan_type != "pseudoScan":
            print(
                "The scan type is '%s' which seems not to be a standard BG/BG scan."
                % compare_scan.scan_type
            )
        ...
        for destination_hash in conf.destination_hash:
            ...
            if not destination_scan:
                raise ValueError(
                    "Destination scan with hash %s not found." % destination_hash
                )

            if compare_scan.distribution != destination_scan.distribution:
                print(
                    "The distribution types '%s' and '%s' seem not to match."
                    % (compare_scan.distribution, destination_scan.distribution)
                )
                answer = prompt("Do you want to proceed anyways?")
                if not answer:
                    return
            ...
            query = (
                session.query(dbtools.Result.event_class)
                .distinct()
            ...
            event_classes = super_filter(
                event_classes,
                inclusion_patterns=conf.filter,
                exclusion_patterns=conf.veto,
            )

            print(
                "Processing %d / %d event classes."
                % (len(event_classes), N_before_filtering)
            )

            insert = dbtools.CorrectedResult.__table__.insert().prefix_with(
                "OR REPLACE"
            )
            ...
            for event_class in event_classes:
                ...
                query = session.query(
                    dbtools.Result.round, dbtools.Result.score
                ).filter(
                    dbtools.Result.hash == compare_scan.hash,
                    dbtools.Result.event_class == event_class,
                )
                ...
                if destination_scan == compare_scan:
                    corrected_p_values = get_pseudo_p_tilde_list(
                        pseudo_p_values, correct_if_zero=False
                    )
                    corrected_tuples = zip(pseudo_round_indices, corrected_p_values)
                else:
                    query = session.query(dbtools.Result).filter(
                        dbtools.Result.hash == destination_scan.hash,
                        dbtools.Result.event_class == event_class,
                    )
                    corrected_tuples = [
                        (
                            result.round,
                            calc_p_tilde(
                                pseudo_p_values, result.score, correct_if_zero=False
                            ),
                        )
                        for result in query
                    ]
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Calculate p-tilde values on results from a database"
    )

    parser.add_argument(
        "--filter",
        help="Only process event classes that match this filter",
        type=str,
        default=("*",),
        nargs="*",
    )
    parser.add_argument(
        "--veto",
        help="Veto event classes matching this pattern",
        type=str,
        default=tuple(),
        nargs="*",
    )
    parser.add_argument(
        "--database",
        help="Path to SQLite database to work on",
        type=str,
        default=dbtools.default_db_name,
    )
    parser.add_argument(
        "compare_hash", help="Hash to compare to (usually BG MC)", type=str
    )
    parser.add_argument(
        "destination_hash", help="p-values that will be corrected", type=str, nargs="+"
    )
