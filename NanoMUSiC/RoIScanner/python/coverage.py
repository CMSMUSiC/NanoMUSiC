...
class CoverageResult:
    ...
    def coverage_pratio(self):
        return (
            math.log10(self.p_true / self.p_claim_corrected)
            if self.p_true > 0
            else np.inf
        )
    ...
    def print_report(self):
        ...
        print(
            "\tTested/not tested:                {:d}/{:d}".format(
                self.n_toys - self.n_nottested, self.n_nottested
            )
        )
        print("\tInvalid (p == 0):                 {:d}".format(self.n_invalid))
        print(
            "\tClaimed type-1 error rate:        {:.04f} (Z_claim  = {:.03f})".format(
                self.p_claim, self.Z_claim
            )
        )
        print(
            "\tAsymetry correction factor:       {:.03f}".format(
                self.asymmetry_correction
            )
        )
        print(
            "\tCorrected claimed values:         {:.04f} (Z_claim' = {:.03f})".format(
                self.p_claim_corrected, self.Z_claim_corrected
            )
        )
        print(
            "\tExpected significant:             {:.02f}".format(
                self.n_toys * self.p_claim_corrected
            )
        )
        print("\tObserved significant:             {:d}".format(self.n_significant))
        print(
            "\tTrue type-1 error rate:           {:.04f} (Z_true   = {:.03f})".format(
                self.p_true, self.Z_true
            )
        )
...
def compute_p(n_obs, n_exp, sigma_exp, prior):
    ...
    if p < 0:
        ...
        p = pCalc.c_compute_p_convolution(
            n_obs, n_exp, sigma_exp, real_prior, debugLevel=0
        )
    return p
...
def coverage(
    ...
    if prior == "normal":
        exp_distribution = scipy.stats.truncnorm(
            -1.0 / rel_err, np.inf, loc=n_true, scale=rel_err * n_true
        )
    ...
    if tested_count < 100:
        print(
            "Warning: only %d of %d pseudo-experiments will be tested."
            % (tested_count, n_toys),
            file=sys.stderr,
        )
    ...
    if not quiet:
        ...
        if False:
            ...
            plt.scatter(
                X[significant],
                Y[significant],
                s=2 * scipy.stats.norm.isf(p[significant]),
                color="red",
                label="Significant",
                lw=0,
            )
            ...
            plt.scatter(
                X[insignificant],
                Y[insignificant],
                s=2 * scipy.stats.norm.isf(p[insignificant]),
                color="green",
                label="Insignificant",
                lw=0,
            )
...
def calculate_grid(
    ...
    args = zip(
        ns_rep,
        sigmas_rep,
        itertools.repeat(Z_claim),
        itertools.repeat(n_toys),
        itertools.repeat(test_case),
...
def plot_grid(
    ...
    pcol = plt.pcolormesh(
        cell_N, cell_S, coverage.T, cmap=cmap, vmin=vmin, vmax=vmax, rasterized=True
    )
...
class CoverageComputer:
    ...
    @cmd.command
    def benchmark(
        self, prior="lognormal", n=10000, N=5, n_obs=10.0, n_exp=8.0, rel_err=0.2
    ):
        durations = []
