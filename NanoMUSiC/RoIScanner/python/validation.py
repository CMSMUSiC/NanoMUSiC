...
def validate(filename):
    ...
    subprocess.check_call(
        [executable, "-j", json_file, "-o", result_file], stdout=DEVNULL, stderr=DEVNULL
    )
    ...
    if len(result["ScanResults"]) == 0:
        if (desired_p - 1.0) < 0.00001:
            return "noToOne"
        fail_reason = "no result"
    elif desired_p == 0.0:
        fail_reason = "8TeVZero"
    else:
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Validate 8-TeV scan results with the 'new' 13-TeV scanner."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=cpu_count(),
        help="Number of CPU cores to use for validation.",
    )
