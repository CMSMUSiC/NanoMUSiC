...
def main():
    parser = argparse.ArgumentParser(
        description="Converter for 8TeV-MUSiC pickle files (*.pkl) to 13TeV-MUSiC JSON job descriptions."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pickle file containing the 8 TeV scanner configuration (usually called 'config.pkl').",
    )
    parser.add_argument(
        "--out", type=str, default="./", help="Directory for JSON job files."
    )
...
def convert(filename, result_directory, configfile=None):
    ...
    for bin_index in range(N_bins):
        ...
        for uncert in source.bg_errorLists[bin_index]:
            ...
            value = float(value)
            if name in ("JES", "Uncl", "MuoES", "EleES", "GamES"):
                continue
            uncerts[name] = value
        # print( uncerts )
