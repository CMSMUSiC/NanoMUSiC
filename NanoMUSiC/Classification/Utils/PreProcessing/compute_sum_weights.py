...
def main():
    parser = argparse.ArgumentParser(
        description="Compute sum of generator weights for MC histograming"
    )
    parser.add_argument("analysis_config", help="Analysis config file path.")

    args = parser.parse_args()

    configs = tomli.loads(Path(args.analysis_config).read_text(encoding="utf-8"))
    total_samples = 0
