...
class Outputs
{
  ...
  public:
    ...
    std::array<std::array<float, Weight::kTotalWeights>, kTotalVariationsAndShifts> weights_nominal;
    std::array<std::array<float, Weight::kTotalWeights>, kTotalVariationsAndShifts> weights_up;
    std::array<std::array<float, Weight::kTotalWeights>, kTotalVariationsAndShifts> weights_down;
    ...
    void set_event_weight(unsigned int idx_var, Weight weight, Shift shift, float value)
    {
        switch (shift)
        {
        case Shift::Up:
            weights_up.at(idx_var).at(weight) = value;
            break;
        case Shift::Down:
            weights_down.at(idx_var).at(weight) = value;
            break;
        default: // nominal
            weights_nominal.at(idx_var).at(weight) = value;
            break;
        }
    }

    void set_event_weight(unsigned int idx_var, Weight weight, float value)
    {
        weights_up.at(idx_var).at(weight) = value;
        weights_down.at(idx_var).at(weight) = value;
        weights_nominal.at(idx_var).at(weight) = value;
    ...
    float get_event_weight(unsigned int idx_var, Weight weight = Weight::kTotalWeights, Shift shift = Shift::Nominal)
    {
        auto nominal_weight =
        ...
        {
        case Shift::Up:
            return weights_up.at(idx_var).at(weight) / weights_nominal.at(idx_var).at(weight) * nominal_weight;
        case Shift::Down:
            return weights_down.at(idx_var).at(weight) / weights_nominal.at(idx_var).at(weight) * nominal_weight;
        default: // nominal
    ...
    void clear_event_tree(unsigned int idx_var)
    {
        ...
        {
            weights_nominal.at(idx_var).at(idx_weight) = 1.;
            weights_up.at(idx_var).at(idx_weight) = 1.;
            weights_down.at(idx_var).at(idx_weight) = 1.;
        }
