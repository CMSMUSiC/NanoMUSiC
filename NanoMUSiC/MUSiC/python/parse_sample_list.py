...
def parse(sample_list_path: str):
    ...
    sample_list = readSamplesFromFile(sample_list_path)
    for sample in tqdm(sample_list):
        sample.parse_name(generators, showers, redlist)

    # read from the cross section toml file
    xsections = tomli.loads(Path(xsection_file_path).read_text(encoding="utf-8"))
    # xsec_miss_samples = []
    ...
    if error_count == 0:
        os.system(f"rm {output_filename} > /dev/null 2>&1")
        with open(output_filename, "w") as new_xsection_file:
