...
def checkTask(task, resubmitJobs, killJobs):
    ...
    task = ce.Task.load(task.directory)
    if len(killJobs) > 0:
...
def clearFinishedJobs(taskList,clearAll=False):
    """remove done jobs from the list
    """
    removeList=[]
    for t in taskList:
        if t.frontendstatus == "RETRIEVED":
            if( clearAll or all([ job.infos["ExitCode"] == "0" for job in t.jobs] ) ):
                removeList.append(t)
    for t in removeList:
        taskList.remove(t)
...
class Overview:
    ...
    def update(self, tasks, resubmitList, killList, nextTaskId):
        ...
        for (taskId, taskOverview, task) in zip(range(len(tasks)), self.taskOverviews, tasks):
            statusnumbers=task.job_status_dict()
            ...
            if nextTaskId == taskId:
                ...
                icon = " "
            cells = [icon, task.name, task.frontendstatus, strperformance, statusnumbers['total'], statusnumbers['PENDING']+ statusnumbers['IDLE']+statusnumbers['SUBMITTED']+statusnumbers['REGISTERED'], statusnumbers['RUNNING'], statusnumbers['REALLY-RUNNING'], statusnumbers['ABORTED'], statusnumbers['FAILED'], statusnumbers['COMPLETED'], statusnumbers['good'], statusnumbers[None]+statusnumbers["None"], statusnumbers['RETRIEVED']] #Yannik changed from DONE-OK and DONE_FAILED
            ...
            for job in task.jobs:
                ...
                try:
                    ...
                    jobreturncode = ""
                try:
                    #jobsince = datetime.datetime.fromtimestamp((job.infos["history"][-1][1])).strftime('%Y-%m-%d %H:%M:%S')#Yannik added this line
                    jobsince = datetime.datetime.fromtimestamp(int(job.infos["history"][-1][1])).strftime('%Y-%m-%d %H:%M:%S')# Yannik commented it out
                    #jobsince = job.infos["history"][-1][1] #Yannik added this line
                ...
                if job.node_id in resubmitList[taskId] or job.node_id in killList[taskId]:
        ...
        if totalstatusnumbers['good'] + totalstatusnumbers['bad'] == 0:
            ...
            strperformance = '{0:>6.1%}'.format(performance)
        cells = ["", "TOTAL", "", strperformance, totalstatusnumbers['total'], totalstatusnumbers['PENDING']+ totalstatusnumbers['IDLE']+totalstatusnumbers['SUBMITTED']+totalstatusnumbers['REGISTERED'], totalstatusnumbers['RUNNING'], totalstatusnumbers['REALLY-RUNNING'], totalstatusnumbers['ABORTED'], totalstatusnumbers['FAILED'], totalstatusnumbers['COMPLETED'], totalstatusnumbers['good'], totalstatusnumbers[None]+totalstatusnumbers["None"], totalstatusnumbers['RETRIEVED']] #Yannik changed from DONE-OK and from DONE-FAILED
    ...
    def level2(self, passphrase):
        ...
        if self.tasks[self.currentTask].jobs[self.currentJob].frontendstatus=="RETRIEVED":
            try:
                #x.addFile("stdout",os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].output_directory,"out.txt"))
                x.addFile("stdout",os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].job_directory,"out.txt")) #Yannik changed the 2. argument of os.path.join
            except:
                #x.addText("stdout","could not find stdout "+ os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].output_directory,"out.txt"))
                x.addText("stdout","could not find stdout "+ os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].job_directory,"out.txt")) #Yannik changed the 2. argument of os.path.join
            try:
                #x.addFile("stderr",os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].output_directory,"err.txt"))
                x.addFile("stderr",os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].job_directory,"err.txt")) #Yannik changed the 2. argument of os.path.join
            except:
                x.addText("stderr","could not find stderr")
        elif not self.nogridmon and "rwth-aachen" in self.tasks[self.currentTask].ce_id:
        ...
        x.addFile("jdl file", os.path.join(self.tasks[self.currentTask].directory, self.tasks[self.currentTask].jobs[self.currentJob].job_directory ,self.tasks[self.currentTask].jobs[self.currentJob].jdlfilename)) #Yannik add the 2. argument of os.join
        self.currentView=x
...
def main(stdscr, options, taskList, passphrase):
    ...
    fileHandler = logging.FileHandler("television.log",mode='a')
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    fileHandler.setFormatter(formatter)
    logger.addHandler(handler)
    logger.addHandler(fileHandler)
    logger.info('Initializing television.')
    ...
    certtime=datetime.datetime.now()+datetime.timedelta( seconds= util.voms_proxy_time_left() )
    ...
    while True:
        ...
        if nextUpdate(lastUpdate, updateInterval, nextTaskId).days<0 or waitingForUpdate is not None:
            ...
            if waitingForUpdate is not None:
                ...
                if passphrase:
                    util.voms_proxy_ensure(648000, passphrase=passphrase)
                    certtime=datetime.timedelta(seconds=util.voms_proxy_time_left())+datetime.datetime.now()
                # prepare parameters
        ...
        if c == ord('+'):
            ...
            resubmitByStatus(taskList, resubmitList, ["None", None, "NONE"], overview)
            overview.update(taskList, resubmitList, killList, nextTaskId)
        elif c == ord('5'):
    ...
    if options.dump:
        ...
        if options.proxy:
            ...
            if passphrase=="":
                ...
                util.voms_proxy_create(passphrase=passphrase)
