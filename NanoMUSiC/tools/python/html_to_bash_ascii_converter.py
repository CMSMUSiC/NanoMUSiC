...
if __name__ == '__main__':
    ...
    file_name = raw_input("Give me a file name to convert!")
    file_out  = raw_input("Give me an output file name!")
    ...
    for item in array:
        if "<span" not in item:
            ...
            dummy = dummy.replace('\n','')
            while True:
                dummy = dummy.replace('</span>','\033[0m')
                dummy2 = dummy[dummy.find('<span'):dummy.find(';">')+3]
                arg = dummy[dummy.find(": #")+3:dummy.find(';">')]
                short, rgb = rgb2short(arg)
                dummy = dummy.replace(dummy2,'\\033[38;5;%sm' % (short))
                if "<span" not in dummy:
                    new_array.append('print ("'+dummy+'")\n')
                    break
