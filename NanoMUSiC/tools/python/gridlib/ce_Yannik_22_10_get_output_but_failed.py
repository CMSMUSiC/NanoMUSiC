...
class Job(TaskSettings):
    ...
    def submit(self):
        ...
        for i in xrange(50):
            ...
            if 'FATAL' in stdout:
                ...
                self.frontendstatus = 'FAILED' #Every DONE-FAILED has been changed with FAILED
                break
            # ftp failure
            ...
            #print "***** CONDOR_SUBMIT - verbose OUTPUT ********"
            #print stdout #Yannik add
            #print "***** END **********"
            #print "***** CONDOR_Q - grid OUTPUT ********"
            test_q_output = self.test_cond_status()#Yannik add
            #print test_q_output #Yannik add
            #print "***** END **********"
            ...
            for line in stdout.splitlines(): 
                ...
                if 'Proc' in line and 'Id' not in line:             #Yannik add
                    ...
                    tr2 = tr1.replace(':','')
                # Yannnik first try
                #if 'Iwd =' in line:             #Yannik add
                 #   tempID = line.split('/')
                  #  tr1 = tempID[len(tempID) -1]
                   # tr2 = tr1.replace('"','')
                #if 'https://' in line: #Yannik commented it out. THIS IS DEFAULT LINE
                    #print "***********  LOR DEBUG ce.py LINE 276 inside if https  *********"
                    #print line #LOR DEBUG
                    #print tr2.strip()
                    #print "************** LOR DEBUG  ce.py Line 278 inside if https END!!!!!!   ***************************"
                    #self.job_id = line.strip() #Yannik comment it out.
    ...
    def retrieve_output(self):
        print("*************************************  TRYING TO RETRIEVE OUTPUT ce.py Line 355 *************************") #Yannik add this
    ...
    def purge(self):
        ...
        #Yannik commented out all the following line and he added the last line
...
def parse_status_multiple_l1(stdout):
    ...
    #**** END *******
    print "**********      STARTING parse_ MULTIPLE  status.... LINE 582 ce.py    **********"
    result = dict()
    job_id = None
    ln2= list()
    ln1= ''
    ...
    for line in stdout.splitlines():
        ...
        if len(ln2) < 3:
            ...
            print '######## ERROR: status lenght is less then 0 ############'
        # END
        print("***** Job PARSE_MULTIPLE_L1 status******")
        print(status)
        print("**** END STATUS")
        result[job_id] = dict()
        result[job_id]['history'] = list()
        ...
        print("***** LEN(RESULT) INSIDE MULTIPLE_PARSE *****")
        print(len(result[job_id]))
        print("**** END STATUS")
...
class Task(TaskSettings):
    ...
    def _update_status_of_jobs(self, joblist=None, retry=0):
        ...
        for job in joblist if joblist is not None else self.jobs:
            print("**************************** job fronrendstatus  Line 1035 ***********************") #Yannik add this print
            print(job.frontendstatus)
            print("**************************** job status  INSIDE UPDATE in the first for Line 1037 ***********************") #Yannik add this print
            print(job.status)
            print("**************************** END  Line 1039 ***********************") #Yannik add this print
        ...
        for job_chunk in job_chunks:
            ...
            if process.returncode is not 0:
                ...
                print ('******    BEF Call pares_status_mult')
                result = parse_status_multiple_l1(stdout)
                print ('**********        AFTER Call pares_status_mult')
                # update infos for each Job
                valid_jobs = 0
                for job in job_chunk:
                    #print("***** LEN(RESULT) INSIDE update_status_multiple *****")
                    #print(len(result[job.job_id]))
                    #print("**** END STATUS")
        ...
        if retry_jobs:
            if retry < 5:
                ...
                #print("************* Line 1083  list(retry_jobs) ****************") #Yannik add this prints
                #print(list(retry_jobs))
                print("************* Line 1086 math.ceil... *********************")
                print(int(math.ceil(min(len(retry_jobs), 50) / (retry + 2))))
                #print("************* Line 1088 (retry + 2) **********************")
                #print((retry + 2))
                print("**********************************************************") #####
    ...
    def retrieve_output(self, connections=1, joblist=None, retry=0):
        ...
        for job in joblist if joblist is not None else self.jobs:
            ...
            if job.status == 'COMPLETED' and job.frontendstatus not in ['RETRIEVED', 'PURGED']: #Yannik add this and commented 1 line above
        ...
        retry_jobs = []
        print(" ****************** debug in Retriveoutput se.py L1225 before glite *****************")
        for job_chunk in job_chunks:
            job_ids = [job.job_id for job in job_chunk if job.job_id is not None]
        ...
        self.save()
        print(" ****************** debug in Retriveoutput se.py L1225 after glite *****************")
