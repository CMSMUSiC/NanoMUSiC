...
auto main(int argc, char *argv[]) -> int
{
    ...
    for (auto &&event : tree_reader)
    {
        ...
        */
        ...
        // CHECK TRIGGER THRESHOLDS FOR HT
        ...
        if (njet >= 2 and nbjet == 0 and (not is_met))
        {
            validation_classes["2J+0BJ+0MET+XJ"]->fill(jets_4vec, bjets_4vec, nelectron, nmuon, met_4vec, weight);
        }
        else if (njet >= 2 and nbjet == 0 and is_met)
        {
            validation_classes["2J+0BJ+1MET+XJ"]->fill(jets_4vec, bjets_4vec, nelectron, nmuon, met_4vec, weight);
        }
    ...
    */
    ...
    validation_classes["2J+0BJ+0MET+XJ"]->dump_outputs();
    validation_classes["2J+0BJ+1MET+XJ"]->dump_outputs();
