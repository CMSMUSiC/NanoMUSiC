// NAME: MUSIC WIDE JETS
        ...
        const float seed_threshold = 250.0; // select seeds
        const float delta_r_threshold = 1.1; // merge non seeds
        const float delta_eta_max_threshold = 1.4; // delta eta max cut
        ...
        if (widejets.size() > 1) // only try delta eta max cut if at least two wide jets exist
        {
            ...
            if (not(all_widejet_delta_eta.at(sortidx_delta_eta.at(0)) < delta_eta_max_threshold)) // check if largest delta eta is below threshold
            {
