...
auto main(int argc, char *argv[]) -> int
{
    ...
    for (auto &&event : tree_reader)
    {
        ...
        */
        ...
        auto pileup_weight = 1.f;

        if (not(is_data))
        {
            pileup_weight = correction_ref->evaluate({unwrap(Pileup_nTrueInt), "nominal"});
            std::cout << pileup_weight << std::endl;
            weight = const_weights * unwrap(gen_weight) * pileup_weight * generator_filter / no_cuts /
        ...
        bool is_good_trigger = unwrap(pass_low_pt_muon_trigger) || unwrap(pass_high_pt_muon_trigger);
