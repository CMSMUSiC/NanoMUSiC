...
###############################
# ONLY CALCULATES NORMALIZATION
normalizewiththis = True
###############################
...
def stacker(
    ...
    norm_idx = []
    corr_fac = {}  # QCD correction factor
    print_norm_fac = -1
    print_norm_fac_err = -1
    printdebug(f"Calculate the QCD normalization from this dataset.")

    # ------ find bins with data in it ------
    last_data = nbins - 1
    while True:
        if datasum[last_data] > 0:
            break
        last_data -= 1
        if last_data < 0:
            last_data = nbins - 1
    ...
    first_data = 0
    while True:
        if datasum[first_data] > 0:
            break
        first_data += 1
        if first_data >= nbins - 1:
            first_data = 0
            break
    ...
    while True:
        ...
        while counter_checked < len_check_interval:  # check next five bins with mc
            ...
            if mcsum[i] > 0:
                counter_checked += 1
                if (
                    total_mc_errors[i] / mcsum[i]
                ) < uncert_threshold:  # rel mc uncert threshold
                    counter_true_condition += 1
            i += 1
        if (
            counter_checked == counter_true_condition
        ):  # check whether all checked bins fulfull requirement
            break
        norm_left += 1  # else go to next
    # right edge, start at other end of the check interval and look backwards
    norm_right = norm_left + len_check_interval
    while True:
        if norm_right >= last_data:
            norm_right = last_data
            break
        # relative total error requirement
        i = norm_right
        counter_checked = 0
        ...
        if (
            ...
            break
        norm_right += 1  # else go to next

    # ------ define index interval ------
    norm_idx = range(norm_left, min([norm_right + 1, last_data + 1]))
    # exception: 1 bin
    if first_data == last_data:
        norm_idx = [first_data, first_data]

    # ------ sum bins inside of normalization interval ------
    datasum_norm = np.sum([datasum[i] for i in norm_idx])
    non_qcd_sum_norm = np.sum([non_qcd_sum[i] for i in norm_idx])
    qcd_sum_norm = np.sum([qcd_sum[i] for i in norm_idx])

    # ------ calculate errors for the regions ------
    # according to the MUSiC AN, all bins have to be treated fully correlated, so first sum all errors over all bins
    # in the interval and only after that combine the different systematics
    # fill empty dict
    syst_sum_norm_samples = {}  # {syst: {sample: sum over all bins in the interval}}
    for syst in systematics:
        syst_sum_norm_samples.update({syst: {}})
        for sample in mcsamples:
            syst_sum_norm_samples[syst].update({sample: 0.0})
    # sum all bins in the interval for all systematics seperately (treat bins fully correlated)
    for syst in systematics:
        for category in categories_samples.keys():  # for MC
            for sample in categories_samples[category]:
                for i in norm_idx:
                    syst_sum_norm_samples[syst][sample] += mcsystematics[syst][sample][
                        i
                    ]
    # do the error merging for the samples as above
    syst_qcd_sum_norm = (
        {}
    )  # {syst: combined error over all samples and over the interval}
    syst_non_qcd_sum_norm = (
        {}
    )  # {syst: combined error over all samples and over the interval}
    for syst in systematics:
        s_error_qcd = 0
        ...
        if syst in [
            ...
            for category in categories_samples.keys():
                if category == "QCD":  # for QCD samples
                    for sample in categories_samples[category]:
                        s_error_qcd += syst_sum_norm_samples[syst][sample]
                else:  # for Non-QCD samples
                    for sample in categories_samples[category]:
                        s_error_non_qcd += syst_sum_norm_samples[syst][sample]
        # TREAT ALL SAMPLES UNCORRELATED
        elif syst in [
            "stat",
        ]:
            for category in categories_samples.keys():
                if category == "QCD":  # for QCD samples
                    for sample in categories_samples[category]:
                        s_error_qcd += syst_sum_norm_samples[syst][sample] ** 2
                else:  # for Non-QCD samples
                    for sample in categories_samples[category]:
                        s_error_non_qcd += syst_sum_norm_samples[syst][sample] ** 2
            s_error_qcd = np.sqrt(s_error_qcd)
            s_error_non_qcd = np.sqrt(s_error_non_qcd)
        # ONLY TREAT ONE GROUP CORRELATED
        elif syst in ["xSecOrder"]:
            ...
            for category in categories_samples.keys():
                if (
                    ...
                    temp = 0
                    for sample in categories_samples[category]:
                        temp += syst_sum_norm_samples[syst][
                            sample
                        ]  # assumed correlated for samples of the same category
                    s_error_non_qcd += (
                        temp**2
            ...
            s_error_non_qcd = np.sqrt(s_error_non_qcd)
        # save error value for systematic source
        syst_qcd_sum_norm.update({syst: s_error_qcd})
        syst_non_qcd_sum_norm.update({syst: s_error_non_qcd})
    # combine all different systematic errors together separately for QCD and Non-QCD as above
    # assume the different systematic sources to be uncorrelated
    err_qcd_sum_norm = 0  # holds squared combined errors
    err_non_qcd_sum_norm = 0
    for syst in systematics:
        if syst != "Nominal":
            err_qcd_sum_norm += syst_qcd_sum_norm[syst] ** 2
            err_non_qcd_sum_norm += syst_non_qcd_sum_norm[syst] ** 2
    err_non_qcd_sum_norm = np.sqrt(err_non_qcd_sum_norm)  # combined errors
    err_qcd_sum_norm = np.sqrt(err_qcd_sum_norm)
    # sum all statistical errors for all bins for each data sample
    err_datasum_norm_samples = {}
    for sample in datasamples:
        err_datasum_norm_samples.update({sample: 0.0})
        for i in norm_idx:
            err_datasum_norm_samples[sample] += datastaterrors[sample][i]
    # sum stat errors (combined for all bins) uncorrelated for each sample
    err_datasum_norm = 0
    for sample in datasamples:
        err_datasum_norm += err_datasum_norm_samples[sample] ** 2
    err_datasum_norm = np.sqrt(err_datasum_norm)

    # ------ calculate normalization ------
    corr_fac = (datasum_norm - non_qcd_sum_norm) / qcd_sum_norm
    # error propagation, assume uncorrelated for now
    err_corr_fac = np.sqrt(
        (err_datasum_norm / qcd_sum_norm) ** 2
        + (err_non_qcd_sum_norm / qcd_sum_norm) ** 2
        + ((datasum_norm - non_qcd_sum_norm) * err_qcd_sum_norm / qcd_sum_norm**2)
        ** 2
    )
    if corr_fac < 0:
        corr_fac = -1
    print_norm_fac = corr_fac
    print_norm_fac_err = err_corr_fac

    # ------ save the normalization ------
    printdebug(f"Saving the normalization.")
    norm_filepath = validation_path + "/" + str(year) + "/plots/"
    if savepath != "":
        norm_filepath = validation_path + "/" + str(year) + "/" + savepath + "/plots/"
    if subfolder != "":
        norm_filepath += subfolder + "/"
    norm_filepath += f"QCD_normalization_{classname}_{histname}.toml"
    norm_dict = {}
    norm_dict.update({"normalization": corr_fac})
    norm_dict.update({"error": err_corr_fac})
...
def plotter(args):
    ...
    if datapresent:
        ...
        # print normalization interval if needed
        if normalizewiththis and len(norm_idx) > 0:
            ax[0].axvline(
                edges[norm_idx[0]],
                linewidth=2,
                color="red",
        ...
        if title != "":  # optional custom file title
            figname = title
...
def main():
    print(
        "\n\n📶 [ MUSiC Validation Plotter 4 (calculate QCD normalization and plot interval) ] 📶\n"
    )
