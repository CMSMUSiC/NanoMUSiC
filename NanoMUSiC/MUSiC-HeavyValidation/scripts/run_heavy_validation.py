...
def validation(args):
    ...
    # remove cutflows
    os.system(f"rm -rf validation_outputs/{year}/cutflow_{process}_{year}.root")
...
def main():
    ...
    if args.sample and args.year:
        ...
        if f"das_name_{year}" in task_config[sample].keys():
            ...
            if not (task_config[sample]["is_data"]):
                validation_arguments["xsection"] = 1.0
                validation_arguments["filter_eff"] = 1.0
                validation_arguments["k_factor"] = 1.0
    ...
    print("ERROR: Could not start validation with the provided arguments.")
    exit(-1)
