...
def parse_args():
    ...
    parser.add_argument(
        "-nwt",
        "--normalizewiththis",
        help="Optional: Write normalization information to the file /plots/QCD_normalization.toml of the current subfolder.",
        action="store_true",
    )
...
def stacker(
    ...
    # ---------------------- apply normalizations if required ----------------------
    ...
    if normalizethis != "":
        ...
        try:
            ...
            print_norm_fac = norm_fac
            categories_counts["QCD"] *= float(norm_fac)
            # ERRORS ?!
        except:
            True
    ...
    for category in categories_stacked:
        ...
        if category == "QCD":
            combinedsum.update({category: categories_counts[category]})
        else:
            temp += categories_counts[category]
    combinedsum.update({"Non-QCD": temp})

    # calculate all mc errors and merge them
    mc_errors_allmc = {}  # {systname: merged error for all mc samples}
    ...
    for syst in systematics:
        ...
        mc_errors_allmc.update({syst: s_error})
    ...
    # ---------------------- calculate normalizations if required ----------------------

    norm_idx = []
    if normalizewiththis and np.sum(datasum) > 0 and np.sum(mcsum) > 0:
        ...
        # conditions for left edge
        while True:
            if norm_left >= last_data:
                norm_left = first_data
                break
            # constant enough ratio requirement
            cur_check_fac = []
            i = norm_left + 1
            while len(cur_check_fac) < 5:
                if mcsum[i] > 0 and datasum[i] > 0:
                    cur_check_fac += [datasum[i] / mcsum[i]]
                i += 1
                if i > last_data:
                    break
            if (
                len(cur_check_fac) > 0
                and mcsum[norm_left] > 0
                and datasum[norm_left] > 0
            ):
                if (
                    np.abs(
                        (datasum[norm_left] / mcsum[norm_left]) - np.mean(cur_check_fac)
                    )
                    < 0.2
                ):
                    # statistics requirement
                    if datasum[norm_left] > 0 and np.amax(datasum) > 0:
                        if datasum[norm_left] / np.amax(datasum) > 0.05:
                            break
            # else go to next
            norm_left += 1
        norm_right = norm_left + 1
        # conditions for right edge
        while True:
            ...
            cur_check_fac2 = []
            i = norm_right - 1
            while len(cur_check_fac2) < 5:
                if mcsum[i] > 0 and datasum[i] > 0:
                    cur_check_fac2 += [datasum[i] / mcsum[i]]
                i -= 1
                if i < norm_left:
                    break
            if (
                len(cur_check_fac2) > 0
                and mcsum[norm_right] > 0
                and datasum[norm_right] > 0
            ):
                if (
                    np.abs(
                        (datasum[norm_right] / mcsum[norm_right])
                        - np.mean(cur_check_fac2)
                    )
                    >= 0.3
                ):
                    # check whether this is only one outlier
                    if norm_right + 1 <= last_data:
                        if (
                            np.abs(
                                datasum[norm_right + 1] / mcsum[norm_right + 1]
                                - np.mean(cur_check_fac2)
                            )
                            < 0.3
                        ):
                            # if outlier, go to next and dont break
                            norm_right += 1
                            continue
                    # if it is no outlier, stop here
                    norm_right -= 1
                    break
            # OR statistics requirement
            if datasum[norm_right] > 0 and np.amax(datasum) > 0:
                if datasum[norm_right] / np.amax(datasum) <= 0.05:
                    norm_right -= 1
                    break
            # else go to next
            norm_right += 1
        ...
        while True:
            ...
            if debugcounter == 1000:
                print(classname, histname, norm_left, norm_right)
            # constant enough ratio requirement
            cur_check_fac = []
            i = norm_right
            while len(cur_check_fac) < 5:
            ...
            if (
                ...
                if (
                    np.amax(np.abs(np.array(cur_check_fac) - np.mean(cur_check_fac)))
                    < 0.2
                ):
                    # statistics requirement
                    if datasum[norm_right] > 0 and np.amax(datasum) > 0:
                        if datasum[norm_right] / np.amax(datasum) > 0.01:
                            break
            if norm_right <= first_data:
                norm_right = last_data
                break
            # else go to next
            norm_right -= 1
        ...
        """
        # find bin indices of normalization interval (if norm int is given in x axis units)
        leftidx = 0
        rightidx = nbins - 1
        while edges[leftidx] <= norm_interval[0]:
            leftidx += 1
        while edges[rightidx] >= norm_interval[1]:
            rightidx -= 1
            if rightidx <= 0:
                rightidx = 0
                break
        norm_idx = range(leftidx, min([rightidx + 1, nbins]))
        """
        # sum bins inside of normalization interval
        datasum_norm = np.sum([datasum[i] for i in norm_idx])
        mcsum_norm = np.sum([mcsum[i] for i in norm_idx])
        combinedsum_norm = {}
        combinedsum_norm.update(
            {"Non-QCD": np.sum([combinedsum["Non-QCD"][i] for i in norm_idx])}
        )
        combinedsum_norm.update(
            {"QCD": np.sum([combinedsum["QCD"][i] for i in norm_idx])}
        )
        # ERROR !?
        # calculate normalization
        corr_fac = (datasum_norm - combinedsum_norm["Non-QCD"]) / combinedsum_norm[
            "QCD"
        ]
        err_corr_fac = 0  # ERROR !?
        if corr_fac < 0:
            corr_fac = 1
        print_norm_fac = corr_fac
        # print the class information
        printdebug("----------------------------------------------------")
        printdebug(f"class: {classname}, histogram: {histname}")
        printdebug(
            f"data: {datasum_norm}, mc: {mcsum_norm}, data/mc: {datasum_norm / mcsum_norm}"
        )
        printdebug(f"   category: QCD,       mc counts: {combinedsum_norm['QCD']}")
        printdebug(f"   category: Non-QCD,   mc counts: {combinedsum_norm['Non-QCD']}")
        printdebug(f"   QCD correction factor f=(data-EW)/QCD: {corr_fac}")
    ...
    if datapresent:
        ...
        if normalizethis != "":
            plt.figtext(
                0.3,
                0.983,
                f"QCD normalized with $\\alpha_{{QCD}} = {np.array(float(print_norm_fac)).round(decimals=2)}$",
                fontsize=12,
                ha="left",
            )
        if normalizewiththis:
            plt.figtext(
                0.3,
                0.983,
                f"Obtained QCD normalization: $\\alpha_{{QCD}} = {np.array(float(print_norm_fac)).round(decimals=2)}$",
                fontsize=12,
                ha="left",
            )
...
def main():
    ...
    # normalize with this flag
    global normalizewiththis
    normalizewiththis = False
    if args.normalizewiththis:
