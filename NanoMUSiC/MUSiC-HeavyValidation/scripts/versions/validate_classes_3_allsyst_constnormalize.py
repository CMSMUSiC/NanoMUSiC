...
def parse_args():
    ...
    parser.add_argument(
        "-nt",
        "--normalizethis",
        help="Optional: Normalize the QCD with the information of the file [normalizethis]/plots/classes_QCD_normalization_[classprefix].toml, the subfolder is specified by this argument.",
    )
    parser.add_argument(
        "-nwt",
        "--normalizewiththis",
        help="Optional: Write normalization information to the file /plots/classes_QCD_normalization_[classprefix].toml of the current subfolder.",
        action="store_true",
    )
...
def countplotter(
    ...
    # ---------------------- calculate and apply normalizations if required ----------------------

    # calculate the correction factors
    if normalizewiththis:
        corr_fac = {}  # dict {classname: QCD correction factor}
        print(f"Calculate the QCD normalization from this dataset.")
        for classname in classes_data.keys():
            # print the class information
            printdebug("----------------------------------------------------")
            printdebug(f"class: {classname}")
            printdebug(
                f"data: {classes_data[classname]}, mc: {total_mc[classname]}, data/mc: {classes_data[classname] / total_mc[classname]}"
            )
            printdebug(
                f"   category: QCD,       mc counts: {classes_categories[classname]['QCD']}"
            )
            temp_nonqcdsum = 0
            for category in classes_categories[classname].keys():
                if category != "QCD":
                    temp_nonqcdsum += classes_categories[classname][category]
            printdebug(f"   category: Non-QCD,   mc counts: {temp_nonqcdsum}")
            corr_fac.update(
                {
                    classname: (classes_data[classname] - temp_nonqcdsum)
                    / classes_categories[classname]["QCD"]
                }
            )
            if corr_fac[classname] < 0:
                corr_fac[classname] = 1
            printdebug(f"   QCD correction factor f=(data-EW)/QCD: {corr_fac}")
        print(f"Saving the normalization.")
        norm_filepath = validation_path + "/" + str(args.year) + "/plots/"
        if savepath != "":
            norm_filepath = (
                validation_path + "/" + str(args.year) + "/" + savepath + "/plots/"
            )
        norm_file = open(
            norm_filepath + f"classes_QCD_normalization_{classsuffix}.toml", "w"
        )
        norm_file.write("[normalization]\n")
        for classname in classes_data.keys():
            norm_file.write('"' + classname + '"=' + str(corr_fac[classname]) + "\n")
        norm_file.close()

    # apply previously calculated normalization factors
    if normalizethis != "":
        print(f"Apply the QCD normalization stored in the given file.")
        norm_filepath = validation_path + "/" + str(args.year) + "/plots/"
        if savepath != "":
            norm_filepath = (
                validation_path
                ...
                + normalizethis
                + "/plots/"
                + f"classes_QCD_normalization_{classsuffix}.toml"
            )
        norm_dict: dict[str, Any] = toml.load(norm_filepath)
        norm_factors = norm_dict["normalization"]
        for classname in classes_categories.keys():
            if (
                classname in norm_factors.keys()
            ):  # correct classes for which a correction exists
                classes_categories[classname]["QCD"] *= norm_factors[classname]
    ...
    for classname in classes_data.keys():
        ...
        if cexcludes != "":
            if cexcludes in classname:
                continue
        # mc threshold
        if total_mc[classname] < mcthreshold_classes:
            break
        if n >= 30:
            break
        n += 1
    ...
    if normalizethis != "":
        plt.figtext(
            0.3,
            0.983,
            "QCD normalized with lepton partner class",
            fontsize=12,
            ha="left",
        )
