...
if [ "`tty`" != "not a tty" ]; then
  ...
  SCRIPTDIR=`pwd`
  bash -c "eval \`scram unsetenv -sh\` ; echo \"================= [ BEGIN ] MUSiC environment ==================\" ; source /cvmfs/sft.cern.ch/lcg/views/LCG_105a/x86_64-el9-gcc12-opt/setup.sh ; echo \"ROOT Version\"; root -b -q; export MUSIC_BASE=$SCRIPTDIR; export LD_LIBRARY_PATH=$SCRIPTDIR/lib:\$LD_LIBRARY_PATH ; export PATH=$SCRIPTDIR/bin:\$PATH ; export PATH=$SCRIPTDIR/scripts:\$PATH ; export PATH=$SCRIPTDIR/NanoMUSiC/MUSiC-CRAB:\$PATH ; export ROOT_INCLUDE_PATH=$SCRIPTDIR/NanoMUSiC/MUSiC/include:$SCRIPTDIR/NanoMUSiC/MUSiC:\$ROOT_INCLUDE_PATH  ; env; echo \"================= [ END ] MUSiC environment ==================\" ; nano_music_skimmer --batch --run-config config.toml"
  exit_code=$?
  ...
  # chekc the status of the outputs 
  echo "================= [ BEGIN ] Checking outputs status =================="
  echo "Cutflow ..."
  root -b nano_music.root -e  "_file0->Get<TH1F>(\"cutflow\")->Print(\"all\")" -q
  if [[ "$?" -eq 0 ]]; then
      echo "Yay! Cutflow is available."
  else 
      exit 42
  fi

  echo "NanoMUSiC Tree ..."
  root -b nano_music.root -e  "_file0->Get<TTree>(\"nano_music\")->Print(\"all\")" -q
  if [[ "$?" -eq 0 ]]; then
      echo "Yay! NanoMUSiC tree is available."
  else 
      exit 42
  fi
  echo "================= [ END ] Outputs directory =================="
