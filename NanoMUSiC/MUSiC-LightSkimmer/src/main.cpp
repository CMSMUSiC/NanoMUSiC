...
auto main(int argc, char *argv[]) -> int
{
    ...
    for (auto &&file : files_to_load)
    {
        ...
        // Note: One file in DoubleEG_13TeV_2016APV_D-HIPM_2016APV does not have any event or branches.
        //  This checks whether there is a reasonable amount of data to analyse
        if (                                       //
            dataframe.GetColumnNames().size() != 0 //
            and dataframe.HasColumn("PV_npvsGood") //
            and dataframe.Count().GetValue() != 0  //
        )
        {
                ...
                pre_skimmed_dataframe
                                                                          ...
                                                                          float mc_weight) -> bool
                        {
                            ...
                            if (!configuration.is_data)
                            {
                                if (configuration.generator_filter_key)
                                {
                                    ...
                                    pass_gen_filter =
                                        GeneratorFilters::filters.at(*(configuration.generator_filter_key))(
                    ...
                    .Define(
                        ...
                        [&configuration]() -> std::string_view
                        {
                            ...
                            {
                                return "Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_BadPFMuonDzFilter && Flag_eeBadScFilter && Flag_hfNoisyHitsFilter"sv;
                            }

                            if (configuration.year == Year::Run2017 or configuration.year == Year::Run2018)
                            {
                                return "Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_BadPFMuonDzFilter && Flag_hfNoisyHitsFilter && Flag_eeBadScFilter && Flag_ecalBadCalibFilter"sv;
                            }

                            throw std::invalid_argument(fmt::format(
                                "ERROR: Could not define MET filters bits. The requested year ({}) is invalid.",
                    ...
                    .Define("pass_double_muon_trigger",
                            [&configuration, &pre_skimmed_dataframe]() -> std::string
                            {
                                std::vector<std::string> double_muon_triggers = {};
                                if (configuration.year == Year::Run2016APV or configuration.year == Year::Run2016)
                                {
                                    if (pre_skimmed_dataframe.HasColumn("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ"))
                                    {
                                        double_muon_triggers.push_back("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ");
                                    }
                                    if (pre_skimmed_dataframe.HasColumn("HLT_TkMu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ"))
                                    {
                                        double_muon_triggers.push_back("HLT_TkMu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ");
                                    }
                                    if (pre_skimmed_dataframe.HasColumn("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ"))
                                    {
                                    ...
                                    if (double_muon_triggers.size() == 0)
                                    {
                                        throw std::invalid_argument(
                                            fmt::format("ERROR: Could not define trigger bits. The requested "
                                                        "trigger(s) is/are invalid."));
                                    }
                                    return fmt::format("{}", fmt::join(double_muon_triggers, " or "));
                                }

                                if (configuration.year == Year::Run2017)
                                {
                                    if (pre_skimmed_dataframe.HasColumn("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8"))
                                    {
                                        double_muon_triggers.push_back("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8");
                                    }
                                    if (pre_skimmed_dataframe.HasColumn("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass8"))
                                    {
                                        double_muon_triggers.push_back("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass8");
                                    }
                                    if (double_muon_triggers.size() == 0)
                                    {
                                        throw std::invalid_argument(
                    ...
                    .Define("pass_double_electron_trigger",
                            [&configuration, &pre_skimmed_dataframe]() -> std::string
                            {
