...
class ClassYieldsPlot(ECCollectorConsumerPlot, ClassCountMixin):
    ...
    def __init__( self,
        ...
        super( ClassYieldsPlot, self ).__init__( ec_records,
                                                 config=config,
        ...
        self.ec_records_by_mc_yield = list(reversed(self.ec_records.sort_records('integral')))
        self.ec_records_by_data_yield = list(reversed(self.ec_records.sort_records('data_integral')))
    ...
    def max_yield(self):
        ...
        return max(self.ec_records_by_mc_yield[0]['integral'],
    ...
    def bin_edges(self):
        ...
        bin_edges = np.linspace(min([r['integral'] for r in self.ec_records]), 1 , 20, endpoint=False)
        for i in range(1, int(np.log10(by_yield[0]['integral'])) + 1 ):
            bin_edges = np.concatenate([bin_edges, np.linspace( 10**(i-1) , 10**i , 9, endpoint=False)])
        return bin_edges
    ...
    def yield_mc_hist(self):
        ...
        if self._yield_mc_hist is None:
            ...
            self._yield_mc_hist = ro.TH1F( "yield_mc",
            ...
            for ibin in range(1, self._yield_mc_hist.GetNbinsX() + 1 ):
                for i, record in enumerate(self.ec_records_by_mc_yield):
                    if self._yield_mc_hist.GetBinLowEdge(ibin) <= self.ec_records_by_mc_yield[i]["integral"]:
                        self._yield_mc_hist.SetBinContent(ibin, len(self.ec_records_by_mc_yield) - i)
                        found = True
                        break
    ...
    def yield_data_hist(self):
        ...
        if self._yield_data_hist is None:
            ...
            self._yield_data_hist = ro.TH1F( "yield_data",
            ...
            for ibin in range(1, self._yield_data_hist.GetNbinsX() + 1 ):
                for i, record in enumerate(self.ec_records_by_data_yield):
                    if self._yield_data_hist.GetBinLowEdge(ibin) <= self.ec_records_by_data_yield[i]["data_integral"] and self.ec_records_by_data_yield[i]["data_integral"] > 0:
                        self._yield_data_hist.SetBinContent(ibin, len(self.ec_records_by_data_yield) - i )
                        break
    ...
    def plot(self):
        ...
        self.legend.AddEntry( self.yield_mc_hist, "{} MC classes".format(self.class_type), "L" )
        self.legend.AddEntry( self.yield_data_hist, "{} Data classes".format(self.class_type), "L" )
        self.legend.Draw()
    ...
    def plot_name(self):
        ...
        return self.get_plot_name(self.config,
                                  self.distribution,
    ...
    def get_plot_name(cls,
        ...
        if object_group_tag:
            return  "Class_Yields_{class_type}_{object_group_tag}{suffix}".format(object_group_tag=object_group_tag,
                                                                          **kwargs)
