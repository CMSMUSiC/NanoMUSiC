...
TEventClass::~TEventClass() {
   ...
   for ( auto& procIter : allProcHistoMap ) {
      for ( auto& distTypeMap : procIter.second) {
         if ( distTypeMap.second != 0 ) delete distTypeMap.second;
      }
   }
   ...
   for ( auto& procIter : SystematicsProcHistoMap ) {
      for ( auto& distTypeMap : procIter.second ) {
         for ( auto& histpair : distTypeMap.second ) {
            if ( histpair.second != 0 ) delete histpair.second;
         }
      }
   }
   dropPDFHistograms();
...
void TEventClass::copyValues( const TEventClass& rhs, bool empty )
{
   ...
   m_globalProcessList = set< string >(); // In case this is *not* meant to be an empty EventClass, the variable is taken from rhs. See below.
...
void TEventClass::InitializeHistos( std::string const &process ) {
   ...
   std::map<std::string, std::map< std::string, TH1F* > >  &systHistos =
      SystematicsProcHistoMap[ process ];
   ...
   for( std::pair< std::string , std::vector < double > > distmap : m_distTypeBinInfo){
      ...
      TH1F *temp_Histo = new TH1F( histoName.c_str(),
                                   histoName.c_str(),
      ...
      temp_Histo = new TH1F( histoNameUnweight.c_str(),
                             histoNameUnweight.c_str(),
                             (distmap.second).size() - 1,
                             &(distmap.second)[0] );
      temp_Histo->Sumw2();
      ...
      if( not m_data ) {
         ...
         for( std::string systName : m_systNames ){
               ...
               systHistos[distType].insert(
                  std::pair<std::string, TH1F*>( systName ,
   ...
   for( vector< float >::const_iterator PDFweight = PDFweights.begin(); PDFweight != PDFweights.end(); ++PDFweight ) {
      ...
      for( auto& distmap : m_distTypeBinInfo){
         ...
         PDFhistos[distType].insert(
            std::pair<std::string, TH1F*>( std::to_string( i ),
                                          new TH1F( histoName.c_str(),
                                                    histoName.c_str(),
                                                    (distmap.second).size() - 1,
                                                    &(distmap.second)[0] ) ) );
      }
...
void TEventClass::InitializeResolutions( std::string const &process ) {
   ...
   for( auto binInfo : m_distTypeBinInfo){
      ...
      resolutions.emplace( distType, new TH2F( TString( histoName ),
...
void TEventClass::Fill( std::string const &process,
   ...
   std::map< std::string, TH1F* > &histos = allProcHistoMap[ process ];
   std::map< std::string, TH1F* > &histosUnweighted = allProcHistoMapUnweighted[ process ];
   if( histos.size() == 0 ) InitializeHistos( process );
   // Get PDF variations
   std::map< std::string, std::map< std::string, TH1F* > >&PDFhistos = PDFvariedallProcHistoMap[ process ];
   auto& systhistos = SystematicsProcHistoMap[ process ];
   if( PDFhistos.size() == 0 ) InitializePDFHistos( process, PDFweights );
   double_t usedWeight = weight;
   if( m_data ) usedWeight = 1.;
   for( auto valuepair : values){
      ...
      if(! m_data ){// Fill constant systematics with custom usedWeights
         for(auto systWeight : systWeights){
            ...
            if( systHisto != systhistos[distType].end()){
               systhistos[ distType ][ systWeight.first ]->Fill( valuepair.second,
                                                               usedWeight * systWeight.second );
            }else{
   ...
   if( resolutionHistos.size() == 0 ) InitializeResolutions( process );
   ...
   if( !m_is_filled) m_is_filled = true;
}
...
void TEventClass::FillDifferentialSystematic( std::string const &process,
   ...
   for( auto valuepair : values){
      if( SystematicsProcHistoMap[ process ][ valuepair.first ].find( systName ) == SystematicsProcHistoMap[ process ][ valuepair.first ].end() ){
         stringstream errorMessage;
...
TH2F* TEventClass::getResolutionsHistoPointer( std::string const &process, std::string distType ) {
   if( m_resolutionsMap.find( process ) == m_resolutionsMap.end( ) ) return 0;
   if( m_resolutionsMap[ process ].find( distType ) == m_resolutionsMap[ process ].end( ) ) return 0;
...
std::set< std::string > TEventClass::getSystematicNames(){
    ...
    for( auto name : m_systNames ) systSet.insert( name );
    return systSet;
}

//----- Get Number of histograms per distType--------------------------------------------------

unsigned int TEventClass::getNumOfDistributions(const string process,
   std::map< std::string, std::map<std::string, std::map< std::string, TH1F* > > > &procMap){
   auto iter = procMap.find( process );
   if (iter == procMap.end())return 0;
   else{
...
unsigned int TEventClass::getNumOfResolutionHistos( string const &process ) const {
   ...
   if( iter == m_resolutionsMap.end() ) return 0;
...
void TEventClass::applyReweighting( std::vector< TH1F* > const &hists,
   ...
   if( upper_hist == 0 ) cout << "Empty upper hist class " << GetName() << " "<<distType << " " <<  m_histnamePDFup << endl;
...
void TEventClass::AddToFolder( B *parent, const char* name,
...
void TEventClass::mergeExistingProcess( std::string process, TEventClass *ECtoBeAdded, bool data ){
   ...
   // The new scale factor is the Lumi * XSec / (total number of MC events in all classes that shall be merged in all files).
   //
   ...
   for( auto& distTypeMap : allProcHistoMap[process]) {
      ...
      if( not baseHist) histNotFound( GetName() , process, distTypeMap.first  );
      if( not mergeHist ) histNotFound( ECtoBeAdded->GetName() , process, distTypeMap.first  );
      if( not data ) {
   ...
   for(auto resHistpair : m_resolutionsMap[process]){
      ...
      if( not mergeResolutionHist) histNotFound(process , ECtoBeAdded->GetName(), "Resolution " + resHistpair.first );
   ...
   if( not data  ){
      ...
      for( auto& distTypeMap : SystematicsProcHistoMap[process]) {
         for(auto& histMap : distTypeMap.second){
            ...
            if( not baseHist) histNotFound( GetName() , process, distTypeMap.first );
            if( not mergeHist ) histNotFound( ECtoBeAdded->GetName() , process, distTypeMap.first + histMap.first);
            if( not data ) {
               // "Unscale" the histograms:
               //
      ...
      if( m_pdfInit ) {
         ...
         //
         if( basePDFWeights.size() == 0 and mergePDFWeights.size() == 0 ) return;
            ...
            for( auto& distTypeMap : PDFvariedallProcHistoMap[ process ]){
               for(auto& histMap : distTypeMap.second ){
                  ...
                  if( not basePDFHist or not mergePDFHist ) {
                     stringstream errorMessage;
                     if( not basePDFHist ) errorMessage << "In base EventClass '" << GetName() << "' ";
                     else                  errorMessage << "In EventClass to be added '" << ECtoBeAdded->GetName() << "' ";
                     errorMessage << "in process '" << process << "' ";
...
void TEventClass::mergeNewProcess( std::string process, TEventClass *ECtoBeAdded, bool data ){
   ...
   if( not data ) {
      ...
      for(auto& distTypeMap :  systematicsProcHistoMapAddedEC[process]){
         for(auto& histMap : distTypeMap.second){
            ...
            if( not addHist ) histNotFound( GetName(), process, distTypeMap.first + " " + histMap.first) ;
            if( not data ) {
      ...
      if( m_pdfInit ) {
         ...
         if( mergeWeights.size() > 0 ) setPDFWeights( process, mergeWeights );
   ...
   for(auto& distTypeMap :  allProcHistoMapAddedEC[process]){
      ...
      if( not addHist ) histNotFound( GetName(), process, distTypeMap.first );
...
void TEventClass::addEventClass( TEventClass *ECtoBeAdded, std::set< std::string > &processesToMerge, bool data ) {
   ...
   //
   if( not data ) checkLumi( this, ECtoBeAdded );
      ...
      for( auto& distReqMap : m_distTypeMinsRequire){
         if( distReqMap.second != ECtoBeAdded->getDistTypeMinRequire( distReqMap.first ) ){
            ...
            err << "BASE EC: "<< distReqMap.first <<" = " << distReqMap.second
                << ", MERGE EC: "<< ECtoBeAdded->getDistTypeMinRequire( distReqMap.first ) <<" = " << ECtoBeAdded->getDistTypeMinRequire( distReqMap.first ) << endl;
