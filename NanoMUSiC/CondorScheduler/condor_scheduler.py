...
class CondorScheduler:
    def __init__(self, output_folder = "", input_folder = "", prologue = ""):
        ...
        if input_folder !="":
            ...
            tar_command = r'tar --exclude="*.log" --exclude="crab.log" --exclude="raw_config.toml" --exclude="crab_nano_music_date_*" --exclude=\
"crab_music_pset.py" --exclude="task.tar.gz" --exclude="CMSSW_*" --exclude="__pycache*" --exclude="build" --exclude="docs_BKP"\
 --exclude="docs" --exclude="crab_nano_music_date_*" --exclude="NanoMUSiC/tools" --exclude="NanoMUSiC/PxlAnalyzer" --exclude="\
*.root" --exclude="NanoMUSiC/PlotLib" --exclude="NanoMUSiC/MUSiC-Configs" --exclude="NanoMUSiC/MUSiC-RoIScanner" --exclude="Na\
noMUSiC/MUSiC-Utils" --exclude="NanoMUSiC/MUSiC-CRAB/crab_nano_music_DYJetsToLL*" -zcvf task.tar.gz __TAR_FOLDER__/*'
            tar_command = tar_command.replace("__TAR_FOLDER__", input_folder)
            os.system(tar_command)
            print("")


    def submit_task(self, executable_script, arg_list):
        base_jdl = """
	universe     = vanilla
	getenv       = true
	...
	Error        = $(LogDirectory)/condor-$(CLUSTER)_$(PROCESS).err
	Stream_Output = true
	Stream_Error = true
	...
	"""
        ...
        string_arg_list = []
        for var in arg_list:
            string_arg_list.append(str(var))
        executable = "cd $TMP; working_dir=$RANDOM; mkdir $working_dir; cd $working_dir;"
        if self.input_folder !="":
            executable += f"/net/software_t2k/tools/cccp/cccp {self.current_dir}/task.tar.gz .; "
            executable += r"tar -zxf task.tar.gz;"
        
        executable += f"{self.prologue}; "
        executable += f"{executable_script} {' '.join(string_arg_list)}"
        os.mkdir(f"{self.output_file_path}/job_id_{self.job_id}")
        with open(f"{self.output_file_path}/job_id_{self.job_id}/executable.sh", 'w') as f:
            f.write(executable)
        os.system(f"chmod +x {self.output_file_path}/job_id_{self.job_id}/executable.sh")
            
        base_jdl = base_jdl.replace("__EXECUTABLE__",f"{self.output_file_path}/job_id_{self.job_id}/executable.sh")
        print (base_jdl)
        ...
        self.job_id += 1
        os.system(f"condor_submit {tmp.name}")
