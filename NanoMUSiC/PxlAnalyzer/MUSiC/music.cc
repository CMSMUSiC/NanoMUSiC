...
//    int b = 4;
//    std::function<int(int)> bar = [&](int a) {
...
//    cout << "Compiling foo_C.so...\n";
//    system("rm -rf foo_*");
//    system("root -l -b -q -e \".L foo.C+\"");

//    // open the library
//    cout << "Opening foo_C.so...\n";
//    void *handle = dlopen("./foo_C.so", RTLD_LAZY);
...
//    // load the symbol
//    cout << "Loading symbol foo...\n";
//    typedef void (*foo_t)(std::function<int(int)> , int);
...
//    if (dlsym_error)
//    {
//       cerr << "Cannot load symbol 'foo': " << dlsym_error << '\n';
//       dlclose(handle);
//       return 1;
//    }
...
int main(int argc, char *argv[])
{
   ...
   genericOptions.add_options()("output,o", po::value<std::string>(&outputDirectory), "Output directory");
   genericOptions.add_options()("config,c", po::value<std::string>(&FinalCutsFile)->required(), "The main config file");
   genericOptions.add_options()("year,y", po::value<std::string>(&year)->required(), "Run2 sample year [2016APV, 2016, 2017 or 2018].");
   genericOptions.add_options()("process,d", po::value<std::string>(&process)->required(), "Dataset name, as generated by \"parse_sample_list_nanoaod.py\".");
   genericOptions.add_options()("dataset,d", po::value<std::string>(&dataset)->required(), "DAS Dataset path.");
   genericOptions.add_options()("input,i", po::value<std::vector<std::string>>(&input_files)->required(), "A list of NanoAOD (.root) files to analyze");
   genericOptions.add_options()("num,n", po::value<int>(&numberOfEvents), "Number of events to analyze.");
   genericOptions.add_options()("skip,s", po::value<int>(&numberOfSkipEvents), "Number of events to skip.");
   genericOptions.add_options()("debug,d", po::value<int>(&debug), "Set the debug level.\n0 = ERRORS,\n1 = WARNINGS,\n2 = INFO, 3 = DEBUG,\n4 = EVEN MORE DEBUG");
   genericOptions.add_options()("cacheread,cr", po::value<bool>(&cacheread), "Should TFile be \"CACHEREAD\"? (Default: true)");
   ...
   {
      std::cerr << "ERROR: year should be 2016APV, 2016, 2017 or 2018" << std::endl;
      return 1;
   }
