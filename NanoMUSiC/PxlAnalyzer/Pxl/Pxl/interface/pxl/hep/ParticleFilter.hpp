...
#include "Pxl/Pxl/interface/pxl/core/Filter.hpp"
...
class ParticlePtEtaNameCriterion : public FilterCriterionInterface<Particle>
{
  public:
    ...
    ParticlePtEtaNameCriterion(const std::string &name, double ptMin = 0.0, double etaMax = 0.0)
    ...
    {
        if ((_name != "" && pa.getName() != _name) || (_ptMin > 0.0 && pa.getPt() < _ptMin) ||
...
{
  public:
    ParticlePtCriterion(double ptMin = 0.0) : _ptMin(ptMin)
    {
    }
