...
class PXL_DLL_EXPORT ObjectOwner
{
  public:
    ...
    template <class objecttype> objecttype *findCopyOf(const Relative *original) const // goes via CopyHistory & casts
...
template <class objecttype> const ObjectOwnerTypeIterator<objecttype> &ObjectOwnerTypeIterator<objecttype>::operator++()
{
    if (_iter != _containerRef->end())
        do
            _iter++;
...
template <class objecttype> inline objecttype *ObjectOwnerTypeIterator<objecttype>::operator*()
{
    return _iter == _containerRef->end() ? 0 : dynamic_cast<objecttype *>(*_iter);
}

template <class objecttype> inline bool ObjectOwnerTypeIterator<objecttype>::operator==(const_iterator iter)
{
    return (_iter == iter);
}
