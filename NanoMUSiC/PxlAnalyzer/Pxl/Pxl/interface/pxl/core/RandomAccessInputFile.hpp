...
#include "Pxl/Pxl/interface/pxl/core/InputFile.hpp"
...
{
  public:
    RandomAccessInputFile() : InputFile(), _currentIndex(0), _eventCount(0), _knowEventCount(false)
    {
    }

    RandomAccessInputFile(const std::string &filename)
        : InputFile(filename), _currentIndex(0), _eventCount(0), _knowEventCount(false)
    {
    }
