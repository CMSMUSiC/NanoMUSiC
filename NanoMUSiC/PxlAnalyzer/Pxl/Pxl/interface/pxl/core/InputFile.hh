...
{
  public:
    InputFile() : InputHandler(), _stream(), _reader(_stream)
    {
    }

    InputFile(const std::string &filename) : InputHandler(), _stream(), _reader(_stream, ChunkReader::seekable)
    {
        this->open(filename);
    }
  ...
  private:
    InputFile(const InputFile &original) : _stream(), _reader(_stream, ChunkReader::seekable)
    {
    }
