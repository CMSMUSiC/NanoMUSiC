...
#include "Pxl/Pxl/interface/pxl/core/BasicNVector.hpp"
...
double BasicNVector::operator*(const BasicNVector &vec) const
{
    ...
    {
        throw std::runtime_error("Size Mismatch! There is only a scalar product defined for vectors of same size.");
    }
    else
    {
        double result = 0;
...
const BasicNVector &BasicNVector::operator-=(const BasicNVector &vec)
{
    if (vec.getSize() != _size1)
    {
        throw std::runtime_error("Size Mismatch! Only vectors of same size can be substracted.");
    }
    else
    {
        for (size_t i = 0; i < _size1; i++)
...
const BasicNVector &BasicNVector::operator+=(const BasicNVector &vec)
{
    if (vec.getSize() != _size1)
    {
        throw std::runtime_error("Size Mismatch! Only vectors of same size can be added.");
    }
    else
    {
        for (size_t i = 0; i < _size1; i++)
