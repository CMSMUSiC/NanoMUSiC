...
{
    return ((std::fabs(obj1.getX() - obj2.getX()) < DBL_EPSILON) &&
            (std::fabs(obj1.getY() - obj2.getY()) < DBL_EPSILON) &&
            (std::fabs(obj1.getZ() - obj2.getZ()) < DBL_EPSILON));
}
...
{
    if ((std::abs(sqrt(_v[0] * _v[0] + _v[1] * _v[1] + _v[2] * _v[2]) - 1.0)) <= DBL_EPSILON)
    {
        return true;
    }
    else
...
{
    return _v[0] > -EPSILON && _v[0] < EPSILON && _v[1] > -EPSILON && _v[1] < EPSILON;
}
