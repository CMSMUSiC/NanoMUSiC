...
#include "Pxl/Pxl/interface/pxl/core/PluginManager.hpp"
...
std::string ErrorString(DWORD err)
{
    std::string Error;
    LPTSTR s;
    if (::FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, err, 0, (LPTSTR)&s, 0,
                        NULL) == 0)
    {
        // failed
        Error = "unknown error";
...
void PluginManager::loadSharedLibrary(const std::string &filename)
{
    ...
    if (handle == 0)
    {
        if (::GetLastError() == ERROR_MOD_NOT_FOUND)
            ...
            throw std::runtime_error("PluginManager: error loading module'" + filename + "', " +
                                     ErrorString(::GetLastError()));
    }

    INITIALIZE_FUNC init = (INITIALIZE_FUNC)GetProcAddress(handle, "pxl_initialize_plugin");
    if (init == 0)
    {
        logger(LOG_LEVEL_WARNING, "plugin ", filename, " has no 'pxl_initialize_plugin' function!");
        FreeLibrary(handle);
        return;
    }
    ...
    {
        logger(LOG_LEVEL_WARNING, "plugin ", filename, " was build with another version of pxl!");
        FreeLibrary(handle);
        return;
    }
...
void PluginManager::loadSharedLibrary(const std::string &filename)
{
    ...
    if (handle == 0)
        throw std::runtime_error("PluginManager: error while loading module'" + filename + "':" + dlerror());

    INITIALIZE_FUNC init = (INITIALIZE_FUNC)dlsym(handle, "pxl_initialize_plugin");
    if (init == 0)
    {
