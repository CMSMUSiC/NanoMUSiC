...
#include "Pxl/Pxl/interface/pxl/hep/AnalysisFork.hpp"
...
{
    for (ObjectOwnerTypeIterator<AnalysisFork> iter(&getObjectOwner()); iter != getObjectOwner().end(); ++iter)
        (*iter)->beginJob(input);

    for (ObjectOwnerTypeIterator<AnalysisProcess> iter(&getObjectOwner()); iter != getObjectOwner().end(); ++iter)
        (*iter)->beginJob(input);
}
