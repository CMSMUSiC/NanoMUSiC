...
void ConsoleLogHandler::handle(LogLevel level, time_t timestamp, const std::string &module, const std::string &message)
{
    ...
    if (level >= LOG_LEVEL_ERROR)
        std::cerr << sstr.str() << " [" << errorColor << LogLevelToString(level) << endColor << "] " << module << " "
                  << message << std::endl;
    else if (level == LOG_LEVEL_WARNING)
        std::cerr << sstr.str() << " [" << warningColor << LogLevelToString(level) << endColor << "] " << module << " "
                  << message << std::endl;
    else
        std::cout << sstr.str() << " [" << LogLevelToString(level) << "] " << module << " " << message << std::endl;
}
...
LogDispatcher::LogDispatcher() : lowestLogLevel(LOG_LEVEL_ERROR)
{
...
void LogDispatcher::dispatch(LogLevel level, const std::string &module, const std::string &message)
{
    time_t timestamp;
    time(&timestamp);
    for (handlers_t::iterator i = handlers.begin(); i != handlers.end(); i++)
...
LogBuffer::LogBuffer(LogLevel level, const char *module) : level(level), module(module)
{
}
