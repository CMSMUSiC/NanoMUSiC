...
void PDFTool::initPDFTool( pxl::EventView const *genEvtView ){
   ...
   if( not hasWeightsInView( genEvtView ) ){
      ...
      initNNPDFPDF();
      std::cout << " I put m_useLHAwweights as True  ...........  " << std::endl;
...
bool PDFTool::hasWeightsInView( pxl::EventView const *genEvtView ){
   ...
   if ( genEvtView->hasUserRecord( m_labelPdfAsCentral ) ){
      m_pdfAsCombined = false;
      if ( !genEvtView->hasUserRecord( m_labelPdfAsDownShifted ) or
           !genEvtView->hasUserRecord( m_labelPdfAsUpShifted ) )
      {
...
void PDFTool::initNNPDFPDF( ) {
   ...
   for( int setID = m_NNPDFUncertIDRange.first;
        setID <= m_NNPDFUncertIDRange.second; setID++ ){
   ...
   if( m_NNPDFAsUncertIDs.first and m_NNPDFAsUncertIDs.second){
      m_AsSetsNNPDF = std::make_pair( LHAPDF::mkPDF( m_NNPDFAsUncertIDs.first ),
                                      LHAPDF::mkPDF( m_NNPDFAsUncertIDs.second ) );
   }
}
...
void PDFTool::setPDFWeightsLHAPDF( pxl::EventView const *genEvtView ) {
   ...
   if( m_AsSetsNNPDF.first && m_AsSetsNNPDF.second ){
      ...
      std::cout << "if( m_AsSetsNNPDF.first && m_AsSetsNNPDF.second )    "<< " as_weight_down:  "<< as_weight_down << "  as_weight_up  " << as_weight_up  << std::endl;
   }
   m_As_weights = std::make_pair( as_weight_down, as_weight_up);
}

double PDFTool::calcLHAWeight ( float const Q,
                                float const x1,
                                float const x2,
                                int const f1,
                                int const f2,
...
void PDFTool::setPDFWeightsFromView( pxl::EventView const *genEvtView ){
   ...
   //LOR ADDING LINES
   //if ( !genEvtView->hasUserRecord( "originalXWGTUP" ) )
   //  {
   //    std::stringstream err;
   //    err << "In file " << __FILE__ << ", function " << __func__ << ", line " << __LINE__ << std::endl;
   //    err << "User Record 'originalXWGTUP' not found but expected!" << std::endl;
   //    throw Tools::value_error( err.str() );
   //  }
   //double originalXWGTUP = genEvtView->getUserRecord( "originalXWGTUP" );
   //END LOR ADDING
   ...
   if ( m_pdfAsCombined )
   {
	...
	std::cout << "m_pdf_weights.size() == 103   ------->   m_pdf_weights.erase( m_pdf_weights.begin() )   " << std::endl;
	m_pdf_weights.erase( m_pdf_weights.begin() );
      ...
      std::cout << "m_pdfAsCombined    "<< " as_weight_down:  "<< as_weight_down << "  as_weight_up  " << as_weight_up  << std::endl;
   }
   else // not combined
   {
      ...
      std::cout << " NOT m_pdfAsCombined    "<< " as_weight_down:  "<< as_weight_down << "  as_weight_up  " << as_weight_up  << std::endl;
   }
   ...
   //LOR ADDING
   //m_As_weights = std::make_pair( as_weight_down / originalXWGTUP, as_weight_up / originalXWGTUP );
   //END LOR ADDING
   m_As_weights = std::make_pair( as_weight_down / generator_weight, as_weight_up / generator_weight );
   std::cout << " In setPDFWeightsFromView     the weight pair you store at the end is as_weight / generator_weight ..... so    "<< " as_weight_down:  "<< m_As_weights.first << "  as_weight_up  " << m_As_weights.second  << std::endl;
...
void PDFTool::setPDFWeights( pxl::EventView const *genEvtView ){
   ...
   if( not m_init ) initPDFTool( genEvtView );
   // retireve weights
...
std::vector< float > PDFTool::parseArrayFromUserRecord( const std::string& pdfWeightsAsString )
{
   ...
   while ( pos2 != std::string::npos && pos1 != pdfWeightsAsString.length())
   {
      ...
      //cout <<  "pdfWeightsAsString.length():  "<< pdfWeightsAsString.length() << " " << pos1 << " " << pos2 << " " << bla << std::endl;
