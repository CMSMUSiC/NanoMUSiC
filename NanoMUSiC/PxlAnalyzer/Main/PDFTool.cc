...
PDFTool::PDFTool(
    Tools::MConfig const &config,
    unsigned int const debug) : m_pdfInfo(),
                                m_debug(debug),
                                m_init(false),
...
void PDFTool::setPDFWeightsFromView(pxl::EventView const *genEvtView)
{
   ...
   if (m_debug > 3)
   {
      std::cout << " In setPDFWeightsFromView     the weight pair you store at the end is as_weight / generator_weight ..... so    "
                << " as_weight_down:  " << m_As_weights.first << "  as_weight_up  " << m_As_weights.second << std::endl;
