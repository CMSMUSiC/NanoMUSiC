...
ScaleFactor::ScaleFactor( const std::string& particleType,
    ...
    m_scale_factor_file_name( cfg.GetItem< std::string >( particleType + ".ScaleFactor." + scale_factor_name + ".file", "" ) ),
    m_scale_factor_directory_name( cfg.GetItem< std::string >( particleType + ".ScaleFactor." + scale_factor_name + ".dir", "" ) ),
    m_scale_factor_histogram_name( cfg.GetItem< std::string >( particleType + ".ScaleFactor." + scale_factor_name + ".hist", "" ) ),
    ...
    m_max_y( -1. )
{
    ...
    if( m_scale_factor_file_name.empty() ){
        ...
        err << "No file name specified for scale factor " << particleType <<" " <<  scale_factor_name << " " << m_type << std::endl;
    ...
    if( !m_scale_factor_directory_name.empty() ){
        ...
        if ( !direc ){
            std::stringstream err;
            err << "In file " << __FILE__ << ", function " << __func__ << ", line " << __LINE__ << std::endl;
            err << particleType << "getting histogram " << m_scale_factor_histogram_name << "containing scale factors" << std::endl;
            err << "Could not find directory " << m_scale_factor_directory_name << " in file " << m_scale_factor_file_name << std::endl;
            throw Tools::value_error( err.str() );
        }
    ...
    if ( !m_scale_factor_hist ){
        std::stringstream err;
        err << "In file " << __FILE__ << ", function " << __func__ << ", line " << __LINE__ << std::endl;
        err << "Could not find histogram " << m_scale_factor_histogram_name << " for scale factor  "  << scale_factor_name << std::endl;
        throw Tools::value_error( err.str() );
    }
    m_scale_factor_hist->SetDirectory( 0 );
    if( m_scale_factor_directory_name.empty() ) delete direc;
...
{
    if( m_type == "PtEta") return getPtEtaScaleFactor( object );
    else if( m_type == "EtaPt") return getEtaPtScaleFactor( object );
...
{
    if( m_type == "PtEta") return getPtEtaScaleFactorError( object );
    else if( m_type == "EtaPt") return getEtaPtScaleFactorError( object );
    std::cout << "WARNING: Unknown type "<< m_type <<" used for scale factor m_name" << std::endl;
    return 0.;
}

bool ScaleFactor::inXrange( const double x ) const {
    if( x < m_min_x or x > m_max_x ) return false;
    return true;
}
bool ScaleFactor::inYrange( const double y ) const {
    if( y < m_min_y or y > m_max_y ) return false;
    return true;
}
