...
class JetTypeWriter
{
  ...
  public:
    ...
    static double getBTagWeight(const std::map<std::string, std::vector<pxl::Particle *>> &part_map,
                                JetTypeWriter::BTagVariation type, const std::vector<std::string> &names = {"Jet"});
  ...
  private:
    ...
    std::string const
        m_bJet_scale_factor_file_name; // Names fdor files, histograms etc to access the Btag SFs and efficiency
