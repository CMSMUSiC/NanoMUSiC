...
class OldNameMapper
{
    struct ReplacementInfo
    {
        ReplacementInfo(std::string varName, std::string varReplacement, int replacementVersion)
            : variableName(std::move(varName)), replacement(std::move(varReplacement)), version(replacementVersion){};
  ...
  public:
    ...
    std::string getUserRecordName(const pxl::Object *obj, std::string variableName,
                                  ...
                                  std::map<std::string, std::string> &cacheReplacementMap, bool isToplevel = true);
