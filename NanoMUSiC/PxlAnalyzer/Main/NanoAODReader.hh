...
public:
  ...
  T getVal(std::string valueName)
  {
    ...
    try
    {
      ...
      if (GetTypeName(typeid(T)) != type_temp_)
      {
        ...
        throw std::runtime_error("--> ERROR: Trying to read branch \"" + valueName + "\" as \"" + GetTypeName(typeid(T)) + "\" is not the same type defiend for this branch. Expected type is: \"" + fType[valueName] + "\".");
      }
    }
    catch (std::runtime_error &e)
    {
  ...
  std::vector<T2> getVec(std::string vectorName)
  {
    ...
    try
    {
      ...
      if (GetTypeName(typeid(T)) != type_temp_)
      {
        throw std::runtime_error("--> ERROR: Trying to read branch \"" + vectorName + "\" as \"" + GetTypeName(typeid(T)) + "\" is not the same type defiend for this branch. Expected type is: \"" + fType[vectorName] + "\".");
      }
    }
    catch (std::runtime_error &e)
    {
...
inline std::vector<UInt_t> NanoAODReader::getVec<Bool_t>(std::string vectorName)
{
  ...
  try
  {
    ...
    {
      throw std::runtime_error("--> ERROR: Trying to read branch \"" + vectorName + "\" as \"" + GetTypeName(typeid(Bool_t)) + "\" is not the same type defiend for this branch. Expected type is: \"" + fType[vectorName] + "\".");
    }
  }
  catch (std::runtime_error &e)
  {
