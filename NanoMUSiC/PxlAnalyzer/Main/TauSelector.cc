...
TauSelector::TauSelector( const Tools::MConfig &cfg, OldNameMapper *globalOldNameMap ):
   ...
   m_tau_syst_discriminators( Tools::splitString< std::string >( cfg.GetItem< std::string >( "Tau.SystDiscriminators" ), true ) )
{
}
...
int TauSelector::passObjectSelection( pxl::Particle *tau,
                                     double const tauRho,
  ...
  //std::cout << " \n ****** START OBJECT SELECTION FOR 1 TAU************* " << std::endl;
   ...
   if( not isSyst){
     for( std::vector< std::string >::const_iterator discr = m_tau_discriminators.begin(); discr != m_tau_discriminators.end(); ++discr ) {
       ...
       //std::cout << "Tau Discriminator Value TO DOUBLE: " << tau->getUserRecord( *discr ).toDouble() << std::endl;
	...
	//std::cout << " ***************** " << std::endl;
      ...
      for( std::vector< std::string >::const_iterator discr = m_tau_syst_discriminators.begin(); discr != m_tau_syst_discriminators.end(); ++discr ) {
         // In theory all tau discriminators have a value between 0 and 1.
         // Thus, they are saved as a float and the cut value is 0.5.
         // In practice most (or all) discriminators are boolean.
         // See also:
   ...
   if(tau->getUserRecord("decayMode")==5 or tau->getUserRecord("decayMode")==6 ) passDecayMode = false;

   //std::cout << "  ****** END OBJECT SELECTION FOR 1 TAU************* " << std::endl;
   if (passKin  && passID && passDecayMode ){std::cout << " ****** RETURN 0 ************* " << std::endl;
     return 0;}
   if (passKin  && !passID && passDecayMode ){//std::cout << " ****** RETURN 1 ************* " << std::endl; 
     return 1;}
   if (!passKin && passID && passDecayMode){//std::cout << " ****** RETURN 2 ************* " << std::endl; 
     return 2;}
...
void TauSelector::tagJetsAsTau( std::vector< pxl::Particle* > &taus, std::vector< pxl::Particle* > &jets ) const{
   ...
   for( std::vector< pxl::Particle* >::const_iterator jet = jets.begin(); jet != jets.end(); ++jet ) {
      for( std::vector< pxl::Particle* >::const_iterator tau = taus.begin(); tau != taus.end(); ++tau ) {
         if( DeltaR((*jet),(*tau))<0.3 and (*tau)->hasUserRecord("systIDpassed" )  and (*tau)->getUserRecord("systIDpassed" ).toBool() ){
