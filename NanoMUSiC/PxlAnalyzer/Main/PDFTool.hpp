...
class PDFTool
{
  public:
    ...
    pdf::PDFInfo const &getPDFInfo() const
    {
        return m_pdfInfo;
    }
    double calculatePdfAsUncertainty();
    void setPDFWeights(pxl::EventView const *genEvtView);
    std::vector<float> getPDFWeights() const
    {
        return m_pdf_weights;
    };
  ...
  private:
    ...
    double calcLHAWeight(float const Q, float const x1, float const x2, int const f1, int const f2, double prodWeight,
                         LHAPDF::PDF *set);
