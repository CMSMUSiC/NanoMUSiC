...
JetSelector::JetSelector(const Tools::MConfig &cfg, OldNameMapper *globalOldNameMap, const std::string &name) : // Add parent constructor
                                                                                                                ObjectSelector(cfg, globalOldNameMap, name, true),
                                                                                                                // general:
                                                                                                                // Jets:
                                                                                                                m_jet_use(cfg.GetItem<bool>(name + ".use")),
                                                                                                                m_jet_pt_min(cfg.GetItem<double>(name + ".pt.min")),
                                                                                                                m_jet_isPF(cfg.GetItem<bool>(name + ".isPF")),
                                                                                                                m_jet_ID_use(cfg.GetItem<bool>(name + ".ID.use")),
...
int JetSelector::passObjectSelection(pxl::Particle *jet,
                                     double const jetRho,
