...
void EventAdaptor::initEvent( pxl::EventView const *RecEvtView ){
   ...
   for( auto& part : allparticles){
      if( part->getName() == m_met_RecName ) m_met = part;
      if( part->getName() == m_muo_RecName ) m_muo_list.push_back( part );
      if( part->getName() == m_ele_RecName ) m_ele_list.push_back( part );
      if( part->getName() == m_fatjet_RecName ) m_fatjet_list.push_back( part );
   }
}
...
void EventAdaptor::applyCocktailMuons( ) const {
   ...
   for( auto& muon : m_muo_list ) adaptMuon( muon );
}


// This function changes the Muon quantities from "normal" to "cocktail" and also adapts
// MET accordingly
...
void EventAdaptor::adaptMuon( pxl::Particle *muon ) const {
   ...
   if( muon->getUserRecord( "validCocktail" ) &&
       muon->getUserRecord( "ptErrorCocktail" ).asDouble() / muon->getUserRecord( "ptCocktail" ).asDouble() < m_muo_highptid_ptRelativeError_max ) {
...
void EventAdaptor::applyHEEPElectrons( ) const {
   for( auto& ele: m_ele_list ){
      if( ele->getPt() > m_ele_switchpt ) adaptEle( ele );
   }
}
...
void EventAdaptor::adaptEle( pxl::Particle *ele )const{
   ...
   TLorentzVector* temp_lor = new TLorentzVector();
...
void EventAdaptor::adaptFatJet( pxl::Particle* fatjet ) const
{
   ...
   temp_lor.SetPtEtaPhiM( fatjet->getUserRecord( "puppi_pt" ), fatjet->getUserRecord( "puppi_eta" ), fatjet->getUserRecord( "puppi_phi" ), fatjet->getUserRecord( "puppi_mass" ) );
   ...
   fatjet->setUserRecord( "puppi_softDropMassCorrected", fatjet->getUserRecord( "puppi_softDropMass" ).toDouble() * getPUPPIweight( fatjet->getUserRecord( "puppi_pt" ), fatjet->getUserRecord( "puppi_eta" ) ) );
}
...
void EventAdaptor::applyJetMetSmearing( pxl::EventView const *GenEvtView,
   ...
   for( ; part != recJets.end(); ++part ) {
      ...
      {
         // Match gen jet
         const auto matchedJet = m_jet_res.matchGenJet( recJet, gen_jets, 0.4, GenEvtView->getUserRecord( "NumVerticesPU" ).toDouble());
         jetPtCorrFactor = m_jet_res.getJetResolutionCorrFactor( recJet, matchedJet, RecEvtView->getUserRecord( m_jet_rho_label ), GenEvtView->getUserRecord( "NumVerticesPU" ).toDouble(), 0 );
      }
      else if ( recName == m_fatjet_RecName )
      {
         const auto matchedJet = m_fatjet_res.matchGenJet( recJet, gen_jets, 0.8, GenEvtView->getUserRecord( "NumVerticesPU" ).toDouble());
         jetPtCorrFactor = m_fatjet_res.getJetResolutionCorrFactor( recJet, matchedJet, RecEvtView->getUserRecord( m_jet_rho_label ), GenEvtView->getUserRecord( "NumVerticesPU" ).toDouble(), 0 );
      }
      else
         std::cerr << "[WARNING] (EventAdaptor): Unknown RecName '" << recName << "' used. No smearing." << std::endl;
