#ifndef CcEventClass_hh
#define CcEventClass_hh
...
#include "CrossSectionOrderErrorMap.hh"
#include "EventClassFactory/ParticleMap.hh"
#include "ParticleSplittingFunctions.hh"

#define TRIGGERSTREAM

#ifdef TRIGGERSTREAM
#include "TriggerStream.hh"
#endif
...
class CcEventClass : public pxl::AnalysisProcess {
public:
   struct EventInfo {
      double sumpt;
      double minv;
   ...
   CcEventClass( const Tools::MConfig &cutconfig ,
...
private:
   ...
   std::map<std::string, TEventClass*>& getReferenceExclusiveMap(std::string type) {
      if (type == "Gen") return _genEventClasses;
      if (type == "Rec") return _recEventClasses;
   ...
   std::map<std::string, TEventClass*>& getReferenceInclusiveMap(std::string type) {
      if (type == "Gen") return _genInclEventClasses;
      if (type == "Rec") return _recInclEventClasses;
   ...
   std::map<std::string, TEventClass*>& getReferenceJetInclusiveMap(std::string type) {
      if (type == "Gen") return _genJetInclEventClasses;
      if (type == "Rec") return _recJetInclEventClasses;
      std::cout << "Inclusive Map " << type << " not found!!" << std::endl;
      //return an empty dummy event-class-map. gcc complained, I dunno what would have happened
      //if a non-existing event-class is a critical error, we should crash here!
      return *(new std::map<std::string, TEventClass*> );
   ...
   bool CheckEventToList( double const sumPt,
                          double const Minv,
                          double const MET
                          ) const;

   void FillEventList( std::string const &ECname,
