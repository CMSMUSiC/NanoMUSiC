...
void EventClassFactory::CheckForCrossSection(string &process)
{
   ...
   if (m_lastprocess.compare(processKey) != 0)
   {
      ...
      {
         throw std::runtime_error("EventClassFactory::CheckForCrossSection( string process ): No value found for item: '" +
                                  processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      if (!m_XSections.CheckItem(processKey))
      {
         throw std::runtime_error("EventClassFactory::CheckForCrossSection( string process ): No XSecOrder found for item: '" +
                                  processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      if (!m_XSections.CheckItem(processGroupKey))
      {
         throw std::runtime_error("EventClassFactory::CheckForCrossSection( string process ): No processGroup found for item: '" +
                                  processKey + "' in cross section file: '" + m_XSections.GetConfigFilePath() + "'. Please check it!");
      }
      ...
      {
         throw std::runtime_error("EventClassFactory, trying to mix samples from different process groups in one run");
      }
...
//----------------------------------------------------------------------
void EventClassFactory::Fill(pxl::EventView *EvtView,
                        double const weight,
                        EventInfo &event_info,
                        std::string systName)
{
...
bool EventClassFactory::CheckEventToList(double const sumPt,
                                    double const Minv,
...
void EventClassFactory::FillEventList(std::string const &ECname,
