...
class CrossSectionOrderErrorMap {
    public:
        ...
        double at( std::string key ){ return weightMap.at( key ); };
              double& operator[]( std::string key )     { return weightMap[key]; };
        //~ const double& operator[]( std::string key ) const { return weightMap[key]; };
        void emplace( std::string key, double value){ weightMap.emplace( key, value ); };
        std::map< std::string, double>::iterator end(){ return weightMap.end();}
        std::map< std::string, double>::iterator find( std::string key ){ return weightMap.find( key );}
};
