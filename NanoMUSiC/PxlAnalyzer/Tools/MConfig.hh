...
namespace Tools
{
   ...
   public:
      ...
      explicit MConfig(const std::string &path = "",
                       ...
                       const bool warnOverwrite = false) : m_configInit(false),
                                                           m_writeFile(false),
                                                           ...
                                                           m_modifiedImportIdentifier2("as")
      {
         ...
         if (m_commentChar == m_delimiterChar)
            throw std::runtime_error("MConfig::MConfig(...): It is not allowed that m_commentChar == m_delimiterChar !");
   ...
   private:
      ...
      {
         if (absPath.empty())
            throw std::runtime_error("MConfig::Init( const string &path ): string m_configFilePath empty! Opening file failed!");
      ...
      void ScanForImports(std::string &line, const unsigned int lineNum, const Path &filePath)
      {
         ...
         if (substrings[0] == m_importIdentifier)
         {
            ...
            else // expect name modifier
            {
               ...
               {
                  std::stringstream error;
                  error << "MConfig::ScanForImports( std::string line, const unsigned int lineNum, const Path &filePath ): Expected"
               ...
               if (substrings[2] != m_modifiedImportIdentifier1)
               {
                  std::stringstream error;
                  error << "MConfig::ScanForImports( std::string line, const unsigned int lineNum, const Path &filePath ): Expected '"
                        << m_modifiedImportIdentifier1 << "' but found '" << substrings[2] << "' on line: " << lineNum << " in file: '"
                        << filePath << "'. Please check it!";
                  throw std::runtime_error(error.str());
               }
               if (substrings[4] != m_modifiedImportIdentifier2)
               {
                  std::stringstream error;
                  error << "MConfig::ScanForImports( std::string line, const unsigned int lineNum, const Path &filePath ): Expected '"
                        << m_modifiedImportIdentifier2 << "' but found '" << substrings[4] << "' on line: " << lineNum << " in file: '"
                        << filePath << "'. Please check it!";
                  throw std::runtime_error(error.str());
               }
