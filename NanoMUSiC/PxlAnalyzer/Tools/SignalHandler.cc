...
SignalHandler* SignalHandler::handler() {
   ...
   if( m_instance == 0 ) m_instance = new SignalHandler();
   return m_instance;
}
...
void SignalHandler::register_handler( int signum, EventHandler *eh ) {
   ...
   if( was_empty ) m_signal_old_handler[ signum ] = old_sa.sa_handler;
}
...
void SignalHandler::remove_handler( int signum, EventHandler *eh ) {
   ...
   if( m_signal_handlers[ signum ].empty() ) signal( signum, m_signal_old_handler[ signum ] );
}
