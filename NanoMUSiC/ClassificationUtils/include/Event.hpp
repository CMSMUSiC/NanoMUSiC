...
class Event
{
    using EventViewMap_t = std::unordered_map<std::string_view, EventView>::iterator;
    using ConstEventViewMap_t = std::unordered_map<std::string_view, EventView>::const_iterator;
  ...
  public:
    ...
    Event(EventView &&event_view)
        : m_event_views()
    {
        m_event_views.emplace("nominal", event_view);
    }

    auto begin() -> EventViewMap_t
    {
        return m_event_views.begin();
    }

    auto end() -> EventViewMap_t
    {
        return m_event_views.end();
    }
    ...
    auto cend() -> const ConstEventViewMap_t
    {
        return m_event_views.cend();
    }

    auto set_view(const std::string_view &view_name, EventView &&event_view) -> Event &
    {
    ...
    auto add_view(const std::string_view &view_name, EventView &&event_view) -> void
    {
        m_event_views.emplace(view_name, event_view);
    }
