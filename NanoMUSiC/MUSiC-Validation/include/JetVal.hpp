...
class JetVal
{
  public:
    ...
    ADD_TH1F(h_1jet_met, n_energy_bins, min_energy, max_energy);
    ADD_TH1F(h_2jet_met, n_energy_bins, min_energy, max_energy);
    ...
    ADD_TH1F(h_nfermion, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ADD_TH1F(h_1jet_nelectron, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ADD_TH1F(h_1jet_nmuon, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ADD_TH1F(h_1jet_nfermion, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ADD_TH1F(h_2jet_nelectron, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ADD_TH1F(h_2jet_nmuon, n_multiplicity_bins, min_multiplicity, max_multiplicity);
    ...
    auto fill0(unsigned int nBJet, unsigned int nJet, unsigned int nElectron, unsigned int nMuon, std::optional<float> met, float weight) -> void;
    // fill1: fill >=1jet plots
    // fill1(jet_1, nElectron, nMuon, met, weight)
    auto fill1(Math::PtEtaPhiMVector jet_1, unsigned int nElectron, unsigned int nMuon, std::optional<float> met, float weight) -> void;
    // fill2: fill >=2jet plots
    // fill2(jet_1, jet_2, nElectron, nMuon, met, weight)
    auto fill2(Math::PtEtaPhiMVector jet_1, Math::PtEtaPhiMVector jet_2, unsigned int nElectron, unsigned int nMuon, std::optional<float> met, float weight) -> void;
