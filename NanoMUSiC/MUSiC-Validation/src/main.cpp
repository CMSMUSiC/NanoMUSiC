...
auto main(int argc, char *argv[]) -> int
{
    ...
    auto z_to_mu_mu_x = ZToLepLepX(
        fmt::format("{}/z_to_mu_mu_x_{}.root", output_path, std::to_string(std::hash<std::string>{}(input_file))));
    auto z_to_ele_ele_x = ZToLepLepX(
        fmt::format("{}/z_to_ele_ele_x_{}.root", output_path, std::to_string(std::hash<std::string>{}(input_file))));
    auto z_to_mu_mu_x_Z_mass = ZToLepLepX(fmt::format(
        "{}/z_to_mu_mu_x_Z_mass_{}.root", output_path, std::to_string(std::hash<std::string>{}(input_file))));
    auto z_to_ele_ele_x_Z_mass = ZToLepLepX(fmt::format(
        "{}/z_to_ele_ele_x_Z_mass_{}.root", output_path, std::to_string(std::hash<std::string>{}(input_file))));
    ...
    if (this_file->Get<TH1F>("cutflow")->GetBinContent(8) != 0)
    {
        for (auto &&event : tree_reader)
        {
            ...
            if (not(already_processed))
            {
                ...
                auto get_leading =
                    [](RVec<float> pt, RVec<float> eta, RVec<float> phi) -> std::optional<Math::PtEtaPhiMVector>
                {
                    if (pt.size() == 0)
                    {
                        return std::nullopt;
