#include "../include/TauLepNuGammaX.hpp"
#include "Configs.hpp"
...
TauLepNuGammaX::TauLepNuGammaX(const std::string &_analysis_name,
...
auto TauLepNuGammaX::fill(const Math::PtEtaPhiMVector &tau,
                    const RVec<Math::PtEtaPhiMVector> &met,
                    const Math::PtEtaPhiMVector &lepton,
                    const Math::PtEtaPhiMVector &photon,
                    float weight) -> void
{
    h_invariant_mass.Fill((tau + met[0] + lepton + photon).Mt(), weight);
    h_sum_pt.Fill(tau.pt() + met[0].pt() + lepton.pt() + photon.pt(), weight);
...
auto TauLepNuGammaX::dump_outputs() -> void
{
