...
def import_counts(year, sample, savepath):
    ...
    return file_contents["counts"] # returns dictionary: {classname: counts} for the sample
...
def countplotter(args, savepath, datasamples, mcsamples, mcsorted, color_dict, aggregation_dict, years, histproperties, classsuffix):
    mcsamples_classes = {}
    datasamples_classes = {}
    for year in years:
        ...
        for sample in mcsamples:
            samplecounts = import_counts(year, sample, savepath) # dictionary: {classname: counts} for the sample
            mcsamples_classes.update({sample: samplecounts})

        # import data event counts
        printdebug(f"Importing {len(datasamples)} data event counts for year {year}...")
        for sample in datasamples:
            samplecounts = import_counts(year, sample, savepath) # dictionary: {classname: counts} for the sample
            datasamples_classes.update({sample: samplecounts})
    
    # sort samples into class dictionary
    # for mc
    mcclassdict = {} # dictionary: {classname: {sample: classcount}}
    for sample in mcsamples:
        for classname in mcsamples_classes[sample].keys():
            if classname in mcclassdict.keys():
                mcclassdict[classname].update({sample: mcsamples_classes[sample][classname]})
            else:
                mcclassdict.update({classname: {sample: mcsamples_classes[sample][classname]}})
    # for data
    dataclassdict = {} # dictionary: {classname: {sample: classcount}}
    for sample in datasamples:
        for classname in datasamples_classes[sample].keys():
            if classname in dataclassdict.keys():
                dataclassdict[classname].update({sample: datasamples_classes[sample][classname]})
            else:
                dataclassdict.update({classname: {sample: datasamples_classes[sample][classname]}})
    print(f"There are {len(dataclassdict)} event classes for data and {len(mcclassdict)} event classes for mc in total (no differentiation between inclusive/exclusive).")
    ...
    if classsuffix in ["+X","+nJ","BJ"]:
        ...
        mcclassdict = tempmcclassdict.copy() # dictionary: {classname: {sample: classcount}} for all classes that have data > 0
        # print result
        if classsuffix == "BJ":
            classsuffix = ""
        if classsuffix == "+X":
    ...
    print(f"There are {len(dataclassdict)} {specifier}{classsuffix} classes in data and {len(mcclassdict)} {specifier}{classsuffix} classes in mc.")
    ...
    for classname in dataclassdict.keys(): # only use data keys
            if classname in mcclassdict.keys():
                ...
                tempmcclassdict.update({classname: {}}) # fill empty dict if the class is only present in data
    if classsuffix == "BJ":
            classsuffix = ""
    ...
    for classname in classes_categories.keys():
        classes_categories[classname] = {k: v for k, v in sorted(classes_categories[classname].items(), key=lambda item: item[1])} # reorder after minimum value
    ...
    for classname in classes_data.keys():
        classes_data = {k: v for k, v in sorted(classes_data.items(), key=lambda item: item[1], reverse=True)} # reorder after minimum value
        # category with lowest event count first
    ...
    print(f"The <=30 classes with the highest data event counts are:\n{classes_data_toplot}")
    ...
    for classname in classes_data_toplot.keys():
        for category in classes_categories[classname]:
            if classname in classes_categories_toplot.keys():
                classes_categories_toplot[classname].update({category: classes_categories[classname][category]})
            else:
                classes_categories_toplot.update({classname: {category: classes_categories[classname][category]}})
    ...
    for classname in classes_data_toplot.keys():
        ...
        for category in classes_categories_toplot[classname].keys():
            if firstrun:
                tempplot = ax[0].bar(x, classes_categories_toplot[classname][category],
                        width=1, bottom=countsum,
                        color=categories_colors[category])
                barplot += [tempplot]
                barlabel += [category]
            else:
                ax[0].bar(x, classes_categories_toplot[classname][category],
                        width=1, bottom=countsum,
                        color=categories_colors[category])
    ...
    mcerrorplot = ax[0].bar([xstart+i for i in range(len(classes_data_toplot.keys()))], 2*mcerr, width=1, bottom=(mcsum-mcerr),
              fill=False, hatch="xxxxxxxx", linewidth=0, edgecolor="tab:gray")
    ...
    for classname in classes_data_toplot.keys():
        ...
        if firstrun:
            dataplot = ax[0].errorbar(x, classes_data_toplot[classname], yerr=np.sqrt(classes_data_toplot[classname]), xerr=1/2,
                            color="black", marker=".", linestyle="", elinewidth=0.8,
                            capsize=1, markersize=3)
        else:
            ax[0].errorbar(x, classes_data_toplot[classname], yerr=np.sqrt(classes_data_toplot[classname]), xerr=1/2,
                            color="black", marker=".", linestyle="", elinewidth=0.8,
                            capsize=1, markersize=3)
        x += 1
        firstrun = False
    ...
    ax[1].tick_params(axis="x", which="major", top=False, bottom=True, labelbottom=False, labeltop=False, direction="out")
    ax[0].tick_params(axis="x", which="minor", top=False, bottom=False, labelbottom=False, labeltop=False, direction="in")
    ax[1].tick_params(axis="x", which="minor", top=False, bottom=False, labelbottom=True, labeltop=False, direction="in")
    ax[1].set_xticklabels([name+" " for name in list(classes_data_toplot.keys())], fontsize="15", rotation="vertical", color="black", ha="center", va="top", minor=True)

    # set y ticks
    ymax = 3*np.amax([np.amax(mcsum), np.amax([classes_data_toplot[classname] for classname in classes_data_toplot.keys()])])
    nmax = 0
    while(10**nmax < ymax):
        nmax += 1
    ...
    whitespace2 = 0.1
    ylim2 = (np.amax([np.amin([np.amin([data_overmc[i]-dataerr_overmc[i]-whitespace2 for i in divisionidx]), np.amin([1-mcerr_overmc[0,i]-whitespace2 for i in divisionidx])]), 0]),
            np.amax([np.amax([data_overmc[i]+dataerr_overmc[i]+whitespace2 for i in divisionidx]), np.amax([1+mcerr_overmc[1,i]+whitespace2 for i in divisionidx])]))
    ax[1].set_ylim(ylim2)
    ...
    ax[0].legend(plots, labels, loc="center", prop={'size': 12}, bbox_to_anchor=(0.9,0.815), frameon=True)
