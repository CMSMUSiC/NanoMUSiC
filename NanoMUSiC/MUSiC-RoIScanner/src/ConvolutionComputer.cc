...
double sum_p_manual( const double N_SM_integration, const double N_SM, const double N_obs) {
    ...
    // compute sum for both cases, using 1-Poisson (thus implementing only a single sum) can lead to problems in case of very small numbers
    ...
    while( N_obs_inc >= 0 ){ //no Poisson below zero
        ...
        if( one_step <= sum_p * precision ){
            if( direction > 0 ){
                if( N_obs_inc > N_SM_integration ) break;
            } else {
                if( N_obs_inc < N_SM_integration ) break;
            }
        }
...
double calculate_upper_bound( const double x ){
    ...
    if( x == 0 ) return -EXP_MIN;
    //the calculation fails for x ~< 1
    //so return the smallest value we can still nicely calculate
    else if( x < 1 ) return second_part( 1, gsl_sf_lambert_Wm1( first_part( 1 ) ) );
    //calculation fails if x >~ 1e18, so return the maximum for a double
    else if( x > 1e18 ) return std::numeric_limits< double >::max();
    //all corner cases covered, do the calculation
    else return second_part( x, gsl_sf_lambert_Wm1( first_part( x ) ) );
}
...
double calculate_lower_bound( const double x ){
    ...
    if( x < 1.1 ) return std::numeric_limits< double >::min();
    //calculation fails for x >~ 1e18, so return the largest value we can still calculate
    if( x > 1e18 ) return second_part( 1e18, gsl_sf_lambert_W0( first_part( 1e18 ) ) );
    //all corner cases covered, do the calculation
    else return second_part( x, gsl_sf_lambert_W0( first_part( x ) ) );
}
...
// More on P and Q here: https://en.wikipedia.org/wiki/Incomplete_gamma_function#Regularized_Gamma_functions_and_Poisson_random_variables
...
double compute_p_convolution( const double N_obs, const double N_SM, const double sigma_MC, PriorMode prior, const int debugLevel ){
    if( debugLevel > 2 ) {
    ...
    const double peak = ( N_SM - err_sq + sqrt( N_SM*N_SM - 2*N_SM*err_sq + err_sq*err_sq + 4*N_obs*err_sq ) ) /2;
    ...
    if( peak_lower < upper ) lower = std::max( lower, peak_lower );
    if( peak_upper > lower ) upper = std::min( upper, peak_upper );
	...
	if ( convolution < 0. ) {
		if ( std::abs(convolution) < 1e-12 ) {
			...
			warn << "    Small negative p-value probalby due to precision, setting it to a small positive value!" << std::endl;
