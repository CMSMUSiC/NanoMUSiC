...
auto ZToLepLepX::dump_outputs(std::unique_ptr<TFile> &output_file) -> void
{
    ...
    // h_lepton_2_pt.Scale(min_bin_width, "width");
    h_lepton_2_pt.SetDirectory(output_file.get());
    h_lepton_2_pt.Write();

    h_lepton_1_eta.SetDirectory(output_file.get());
    h_lepton_1_eta.Write();

    h_lepton_2_eta.SetDirectory(output_file.get());
    h_lepton_2_eta.Write();

    h_lepton_1_phi.SetDirectory(output_file.get());
    h_lepton_1_phi.Write();

    h_lepton_2_phi.SetDirectory(output_file.get());
    h_lepton_2_phi.Write();
