#include "../include/TauGammaNuX.hpp"
#include "Configs.hpp"
...
TauGammaNuX::TauGammaNuX(const std::string &_analysis_name,
...
auto TauGammaNuX::fill(const Math::PtEtaPhiMVector &tau,
                    const RVec<Math::PtEtaPhiMVector> &met,
                    const Math::PtEtaPhiMVector &photon,
                    float weight) -> void
{
    h_invariant_mass.Fill((tau + met[0] + photon).Mt(), weight);
    h_sum_pt.Fill(tau.pt() + met[0].pt() + photon.pt(), weight);
...
auto TauGammaNuX::dump_outputs() -> void
{
